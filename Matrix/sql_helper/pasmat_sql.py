...
def get_pasmats(chat_id):
    try:
        return SESSION.query(Pasmat).filter(Pasmat.chat_id == str(chat_id)).all()
    finally:
        SESSION.close()
...
def remove_all_pasmats(chat_id):
    if saved_pasmat := SESSION.query(Pasmat).filter(Pasmat.chat_id == str(chat_id)):
        saved_pasmat.delete()
        SESSION.commit()
