...
async def animator(media, mainevent, textevent=None):
    ...
    Olddev = await mainevent.client.download_media(media, Config.TEMP_DIR)
    file = await fileinfo(Olddev)
    h = file["height"]
    w = file["width"]
    ...
    await runcmd(
        f"ffmpeg -to 00:00:02.900 -i '{Olddev}' -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an animate.webm"
    )  # pain
    os.remove(Olddev)
    return "animate.webm"
...
# https://www.tutorialspoint.com/How-do-you-split-a-list-into-evenly-sidev-chunks-in-Python
...
def higlighted_text(
    ...
    mask_size = int((residev_width / text_wrap) + 50)
    list_text = []
    output = []
    output_text = []
    raw_text = text.splitlines()
    for item in raw_text:
        input_text = "\n".join(wrap(item, int((40.0 / residev_width) * mask_size)))
    ...
    for pic_no, list_text in enumerate(texts):
        ...
        source_img = templait.convert("RGBA").resize((residev_width, residev_height))
        if direction == "upwards":
            list_text.reverse()
            operator = "-"
            hight = (
                residev_height - (text_height + int(text_height / 1.2)) + extra_height
            )
        else:
            operator = "+"
