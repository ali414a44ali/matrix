...
class CatConverter:
    async def _media_check(self, reply, dirct, file, memetype):
        ...
        if os.path.exists(catfile):
            os.remove(catfile)
        try:
            devmedia = reply if os.path.exists(reply) else None
        except TypeError:
            if memetype in ["Video", "Gif"]:
                dirct = "./temp/catfile.mp4"
            elif memetype == "Audio":
                dirct = "./temp/catfile.mp3"
            devmedia = await reply.download_media(dirct)
        return catfile, devmedia
    ...
    async def to_image(
        ...
        catfile, devmedia = await self._media_check(reply, dirct, file, memetype)
        if memetype == "Photo":
            im = Image.open(devmedia)
            im.save(catfile)
        elif memetype in ["Audio", "Voice"]:
            await runcmd(f"ffmpeg -i '{devmedia}' -an -c:v copy '{catfile}' -y")
        elif memetype in ["Round Video", "Video", "Gif"]:
            await take_screen_shot(devmedia, "00.00", catfile)
        if mediatype == "Sticker":
            if memetype == "Animated Sticker":
                catcmd = f"lottie_convert.py --frame 0 -if lottie -of png '{devmedia}' '{catfile}'"
                ...
                im = Image.open(devmedia)
                im.save(catfile)
        if devmedia and os.path.exists(devmedia):
            os.remove(devmedia)
        if os.path.exists(catfile):
            if rgb:
    ...
    async def to_webm(
        ...
        catfile, devmedia = await self._media_check(reply, dirct, file, memetype)
        media = await fileinfo(devmedia)
        h = media["height"]
        w = media["width"]
        w, h = (-1, 512) if h > w else (512, -1)
        await runcmd(
            f"ffmpeg -to 00:00:02.900 -i '{devmedia}' -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an '{catfile}'"
        )  # pain
        if os.path.exists(devmedia):
            os.remove(devmedia)
        if os.path.exists(catfile):
    ...
    async def to_gif(
        ...
        if mediatype == "Sticker":
            if memetype == "Video Sticker":
                await runcmd(f"ffmpeg -i '{devmedia}' -c copy '{catfile}'")
            elif memetype == "Animated Sticker":
                await runcmd(f"lottie_convert.py '{devmedia}' '{catfile}'")
        if devmedia.endswith(".gif"):
            await runcmd(f"ffmpeg -f gif -i '{devmedia}' -fs {maxsize} -an '{catfile}'")
        else:
            await runcmd(
                f"ffmpeg -i '{devmedia}' -c:v libx264 -fs {maxsize} -an '{catfile}'"
            )
        if devmedia and os.path.exists(devmedia):
            os.remove(devmedia)
        if os.path.exists(catfile):
