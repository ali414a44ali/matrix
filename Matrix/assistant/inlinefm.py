...
from Matrix.core.decorators import check_owner
from Matrix.helpers import humanbytes, progress
from Matrix.helpers.utils import _devutils
...
# BACK
@blal.tgbot.on(CallbackQuery(pattern="fmback"))
@check_owner
...
# UP
@blal.tgbot.on(CallbackQuery(pattern="fmup_(.*)"))
@check_owner
async def up(event):
...
# DOWN
@blal.tgbot.on(CallbackQuery(pattern="fmdown_(.*)"))
@check_owner
...
# FORTH
@blal.tgbot.on(CallbackQuery(pattern="fmforth_(.*)"))
@check_owner
...
# REMOVE
@blal.tgbot.on(CallbackQuery(pattern="fmrem_(.*)"))
@check_owner
async def remove(event):
    ...
    await asyncio.sleep(1)
    await event.edit(msg, buttons=buttons)
    await _devutils.runcmd(f"rm -rf '{rpath}'")
    await event.answer(f"- المسـار {rpath} تم حذفـه .. بنجـاح ✓")


# SEND
@blal.tgbot.on(CallbackQuery(pattern="fmsend"))
@check_owner
...
# CUT
@blal.tgbot.on(CallbackQuery(pattern="fmcut_(.*)"))
@check_owner
async def cut(event):
...
# COPY
@blal.tgbot.on(CallbackQuery(pattern="fmcopy_(.*)"))
@check_owner
...
# PASTE
@blal.tgbot.on(CallbackQuery(pattern="fmpaste_(.*)"))
@check_owner
