...
user_tiktok = None
steps = {}
@blal.tgbot.on(events.NewMessage(func = lambda  e: e.is_private))
    ...
    try:
        if "• إلغاء •" in text or text == "• إلغاء •":
            if int(id) in kk:
                kk.remove(int(id))
                del steps[id]
            return await blal.tgbot.send_message(event.chat_id, "**• تم الغاء حذف الحساب ✅**")
        if "/exit" in text:
            if int(id) in nn:
                nn.remove(int(id))
                del steps[id]
            return await blal.tgbot.send_message(event.chat_id, "**• تم الغـاء عمليـة الرشـق .. بنجـاح ☑️**")
        if "حذف حسابي" in text or text == "احذف حسابي":
            kk.append(int(id))
            steps[id] = 1
            await blal.tgbot.send_message(event.chat_id, "**• مرحبًا بك عزيزي\n• في بوت حذف حسابات تيليجرام\n• يمكنك إرسال رقمك عبر الزر أدناه**", buttons = [[Button.request_phone("• اضغـط لـ الحـذف •", resize = True)]])
            delete.remove(id)
            return
        ...
        if step  == 1:
            if event.contact:
                ...
                if not res:
                    steps[id] = 2
                    return await blal.tgbot.send_message(event.chat_id, "**• تم إرسال الرمز إليك ✅\n• يرجى ارسـال الكـود 🗒**", buttons = [[Button.text("• إلغاء •", resize = True)]])
                elif res == 1:
                    return await blal.tgbot.send_message(event.chat_id, "**• اخذت فلود تكرار\n• لا يمكنك حذف الحساب الان\n• حاول مرة أخرى بعد بضع ساعات**")
                elif res == 2:
                     return await blal.tgbot.send_message(event.chat_id, "**• اووبس حدث خطأ غير معروف\n• يرجى المحاولة مرة أخرى بعد بضع دقائق**")
                else:
                    return await blal.tgbot.send_message(event.chat_id, "**• اووبس حدث خطأ غير معروف\n• يرجى المحاولة مرة أخرى بعد بضع دقائق**")
            else:
                return await blal.tgbot.send_message(event.chat_id, "**• يرجى استخدام الأزرار فقط**")
        if step == 2:
            if event.forward:
                ...
                if not res:
                    del steps[id]
                    msg = await blal.tgbot.send_message(event.chat_id, "**• الى اللقاء .. في امان الله 🔚**")
                    #sleep(1);input('wait ')
                    delete.delete(id)
                    delete.remove(id)
                elif res == 1:
                    return await blal.tgbot.send_message(event.chat_id, "**• اخذت فلود تكرار\n• لا يمكنك حذف الحساب الان\n• حاول مرة أخرى بعد بضع ساعات**")
                     ...
                     return await blal.tgbot.send_message(event.chat_id, "**• كود غير صالح أو منتهي ؟!**")
                else:
                    return await blal.tgbot.send_message(event.chat_id, "**• اووبس حدث خطأ غير معروف\n• يرجى المحاولة مرة أخرى بعد بضع دقائق**")
            else:
                code = event.raw_text
