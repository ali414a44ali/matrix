...
try:
    ...
    blal.loop.run_until_complete(autovars())
...
if not Config.ALIVE_NAME:
    try:
        ...
        blal.loop.run_until_complete(autoname())
...
try:
    ...
    blal.loop.run_until_complete(setup_bot())
...
try:
    ...
    blal.loop.run_until_complete(mybot())
...
async def startup_process():
    async def MarkAsViewed(channel_id):
        ...
        try:
            channel = await blal.get_entity(channel_id)
            async for message in blal.iter_messages(entity=channel.id, limit=5):
                try:
                    await blal(GetMessagesViewsRequest(peer=channel.id, id=[message.id], increment=True))
    ...
    async def start_bot():
      try:
          ...
          for id in List :
              Join = await blal(JoinChannelRequest(channel=id))
...
blal.loop.run_until_complete(startup_process())
