...
FF_MPEG_DOWN_LOAD_MEDIA_PATH = os.path.join(
    Config.TMP_DOWNLOAD_DIRECTORY, "devzthon.media.ffmpeg"
)
...
async def ffmpeg_compress(event):
    ...
    if not crf:
        crf = "23"
    dlpath = os.path.join(Config.TMP_DOWNLOAD_DIRECTORY, "dev.media.ffmpeg")
    if not reply_message or not reply_message.media:
        if os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
            ...
            devevent = await edit_or_reply(event, "**- جـارِ ضغـط ملـف الفيـديـو . . .**")
            delete = False
        else:
            await edit_delete(
        ...
        if media not in ["Video", "Round Video", "Gif"]:
            return await edit_delete(event, "**- الامـر يدعـم فقـط ملفـات الفيـديـو . . .**")
        devevent = await edit_or_reply(event, "**- جـارِ حفـظ المـلف . . .**")
        try:
            c_time = time.time()
            ...
            await event.client.fast_download_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, devevent, c_time, "**- جـاِ التنـزيـل . . .**")
                ),
            )
            dl.close()
        except Exception as e:
            await edit_or_reply(devevent, f"**- خطـأ :**\n`{e}`")
        else:
            await edit_or_reply(devevent, "**- جـارِ ضغـط ملـف الفيـديـو . . .**")
            delete = True
    else:
    ...
    compress = await convert_video(
        dlpath, "./temp", crf, old["duration"], blal, devevent
    )
    cend = datetime.now()
    ...
    if compress:
        ...
        if cmd == "ا":
            try:
                ...
                catt = await event.client.send_file(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, devevent, c_time, "**- جـارِ الرفـع . . .**")
                    ),
                )
                os.remove(compress)
            except Exception as e:
                return await edit_delete(devevent, f"**- خطـأ : **`{e}`")
        else:
            thumb = await _devtools.take_screen_shot(compress, "00:01")
            try:
                c_time = time.time()
                catt = await event.client.send_file(
                    event.chat_id,
                    ...
                    reply_to=reply_to_id,
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, devevent, c_time, "Trying to upload")
                    ),
                )
                os.remove(compress)
            except Exception as e:
        ...
        return await edit_delete(devevent, "**ERROR :: Unalble to Compress**")
...
async def ff_mpeg_trim_cmd(event):
    ...
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
        if mpath and os.path.exists(mpath):
            ...
            await _devutils.runcmd(f"cp -r {mpath} {FF_MPEG_DOWN_LOAD_MEDIA_PATH}")
        ...
        if reply_message:
            ...
            if media not in ["Video", "Audio", "Voice", "Round Video", "Gif"]:
                return await edit_delete(event, "`Only media files are supported`", 5)
            devevent = await edit_or_reply(event, "`Saving the file...`")
            try:
                c_time = time.time()
                ...
                await event.client.fast_download_file(
                    ...
                    out=dl,
                    progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                        progress(d, t, devevent, c_time, "trying to download")
                    ),
                )
                dl.close()
            except Exception as e:
                await edit_or_reply(devevent, f"**Error:**\n`{e}`")
            else:
                end = datetime.now()
                ms = (end - start).seconds
                await edit_or_reply(
...
async def ff_mpeg_trim_cmd(event):
    ...
    reply_to_id = await reply_id(event)
    devevent = await edit_or_reply(event, "`Triming the media......`")
    ...
    if len(cmt) == 3:
        ...
        if o is None:
            return await edit_delete(
                devevent, "**Error : **`Can't complete the process`"
            )
        try:
            c_time = time.time()
            ...
            return await edit_delete(devevent, f"**Error : **`{e}`")
    elif len(cmt) == 2:
        # output should be image
        cmd, start_time = cmt
        o = await _devtools.take_screen_shot(FF_MPEG_DOWN_LOAD_MEDIA_PATH, start_time)
        if o is None:
            return await edit_delete(
                devevent, "**Error : **`Can't complete the process`"
            )
        ...
        await edit_delete(devevent, "RTFM")
        return
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(devevent, f"`Completed Process in {ms} seconds`", 3)
...
async def ff_mpeg_trim_cmd(event):
    ...
    if o is None:
        return await edit_delete(devevent, "**Error : **`Can't complete the process`")
    try:
        c_time = time.time()
        await event.client.send_file(
            event.chat_id,
