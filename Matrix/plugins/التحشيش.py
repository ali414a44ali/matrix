...
zel_dev = (7291869416, 5426990871)
########################  SOURCE dev ~ BY: ZelZal (@zzzzl1l)  ########################
...
async def _(dev):
    await edit_or_reply (dev, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗧𝞝𝗣 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/Matrix) 🗑♻️𓆪\n**𓍹━─━─━─━─𝗧𝞝𝗣─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
ZelzalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝞝𝗣 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/Matrix) 𓆪\n\n"
...
async def dev(mention):
    user, custom = await get_user_from_event(mention)
    if not user:
        return
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(mth)
    await edit_or_reply(mention, f"**✾╎نـسبـة حبكـم انـت و**  [{devth}](tg://user?id={user.id}) **هـي {devt} 😻♥️**")
...
async def dev(mention):
    ...
    await edit_or_reply(mention, f"**✾╎نسبـة الانوثه لـ**  [{devth}](tg://user?id={user.id}) **هـي {devt} 🤰**")
...
async def permalink(mention):
    ...
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(zid)
    await edit_or_reply(mention, f"**✾╎نسبـة الغبـاء لـ**  [{devth}](tg://user?id={user.id}) **هـي {devt} 😂💔**")
...
async def permalink(mention):
    ...
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(zid)
    await edit_or_reply(mention, f"**✾╎نسبـة الانحـراف لـ**  [{devth}](tg://user?id={user.id}) **هـي {devt} 🥵🖤**")
...
async def permalink(mention):
    ...
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(zid)
    await edit_or_reply(mention, f"**✾╎نسبـة المثليـه لـ**  [{devth}](tg://user?id={user.id}) **هـي {devt} 🤡 🏳️‍🌈.**")
...
async def permalink(mention):
    ...
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(zid)
    await edit_or_reply(mention, f"**✾╎نسبـة النجـاح لـ** [{devth}](tg://user?id={user.id}) **هـي {devt} 🤓.**") 
...
async def permalink(mention):
    ...
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(zid)
    await edit_or_reply(mention, f"**✾╎نسبـة الكراهيـة لـ** [{devth}](tg://user?id={user.id}) **هـي {devt} 🤮.**")
...
async def dev(mention):
    ...
    await edit_or_reply(mention, f"**🚹 ╎ المستخـدم  ⪼ • ** [{devth2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـه ورع القـروب .. بنجـاح😹🙇🏻.** \n**🤵‍♂️ ╎ بواسطـه  :** {my_mention} ")
...
async def dev(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**🚺 ╎ المستخـدم ه ⪼ • ** [{devth2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـها مـزة الكروب .. بنجـاح 🥳💃.** \n**🤵‍♂️ ╎ بواسطـه  :** {my_mention} ")
...
async def permalink(mention):
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(
        mention, f"**🚹 ╎ المستخـدم  ⪼ • ** [{devth2}](tg://user?id={user.id}) \n☑️ **🐈╎ تم رفعـه بـزون .** \n**🤵‍♂️ ╎ بواسطـه  :** {my_mention} "
    )
