...
dev_dev = (7291869416, 7291869416, 7291869416)
...
async def ge(user, event):
    ...
    return user_obj
########################################
@blal.dev_cmd(pattern="رابط الحذف")
async def _(dev):
    await edit_or_reply (dev, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/BDB0B) 🗑♻️𓆪\n**𓍹━─━─━─━──━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف¹  ↬** @LC6BOT\n**✵│بـوت الحـذف²  ↬** @DTeLebot")
########################################
@blal.dev_cmd(pattern="بوت الحذف")
async def _(dev):
    await edit_or_reply (dev, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/BDB0B) 🗑♻️𓆪\n**𓍹━─━─━─━──━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف¹  ↬** @LC6BOT\n**✵│بـوت الحـذف²  ↬** @DTeLebot")
#####################################
@blal.dev_cmd(pattern="رفع جلب(?: |$)(.*)")
...
######################################
@blal.dev_cmd(pattern="رفع بكلبي(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
#####################################
@blal.dev_cmd(pattern="رفع بقلبي(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
########################   Userbot ~ By: ########
@blal.dev_cmd(pattern="رفع جريذي(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
########################   Userbot ~ By:  (@BDB0B)  ########################
MatrixalTHS_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗖𝗼𝗻𝗳𝗶𝗴 𝗩𝗮𝗿𝘀 - اوامـر التحشيش](t.me/BDB0B) 𓆪\n\n"
...
@blal.dev_cmd(pattern="كت(?: |$)(.*)")
async def ahmed(ahmed): # Code Update
    Matrixal = ahmed.pattern_match.group(1)
    zilzal = await ahmed.get_reply_message()
    zel_zal = random.choice(kettuet)
    if not zilzal and not Matrixal: # Code 
        return await edit_or_reply(ahmed, f"**⌔╎{zel_zal}**")
    user, custom = await get_user_from_event(ahmed)
    if not user: # Code 
        return
    devth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    if custom: # 
        devth2 = custom
    me = await ahmed.client.get_me()
    my_first = me.first_name
    my_ahmed = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(ahmed, f"**⌔╎لـ  ** [{devth2}](tg://user?id={user.id}) \n**⌔╎{zel_zal}**")

@blal.dev_cmd(pattern="(نسبه الحب|نسبة الحب)(?: |$)(.*)")
async def dev(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(mth)
    await edit_or_reply(malath, f"**✾╎نـسبـة حبكـم انـت و**  [{devth}](tg://user?id={user.id}) **هـي {devt} 😻♥️**")

@blal.dev_cmd(pattern="(نسبه الانوثة|نسبة الانوثه|نسبه الانوثه|نسبة الانوثة)(?: |$)(.*)")
async def dev(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    if user.id in zel_dev:
        return await edit_or_reply(malath, f"**╮ ❐ عـذراً .. لا يمكننـي اهانـة مطـور السـورس  ❏╰**")
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(zid)
    await edit_or_reply(malath, f"**✾╎نسبـة الانوثه لـ**  [{devth}](tg://user?id={user.id}) **هـي {devt} 🤰**")

@blal.dev_cmd(pattern="(نسبه الغباء|نسبة الغباء)(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(zid)
    await edit_or_reply(malath, f"**✾╎نسبـة الغبـاء لـ**  [{devth}](tg://user?id={user.id}) **هـي {devt} 😂💔**")

@blal.dev_cmd(pattern="(نسبه الانحراف|نسبة الانحراف)(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(zid)
    await edit_or_reply(malath, f"**✾╎نسبـة الانحـراف لـ**  [{devth}](tg://user?id={user.id}) **هـي {devt} 🥵🖤**")

@blal.dev_cmd(pattern="(نسبه المثليه|نسبة المثليه)(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(zid)
    await edit_or_reply(malath, f"**✾╎نسبـة المثليـه لـ**  [{devth}](tg://user?id={user.id}) **هـي {devt} 🤡 🏳️‍🌈.**")

@blal.dev_cmd(pattern="(نسبه النجاح|نسبة النجاح)(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(zid)
    await edit_or_reply(malath, f"**✾╎نسبـة النجـاح لـ** [{devth}](tg://user?id={user.id}) **هـي {devt} 🤓.**") 

@blal.dev_cmd(pattern="(نسبه الكراهية|نسبة الكراهيه|نسبه الكراهيه|نسبة الكراهية)(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    devth = user.first_name.replace("\u2060", "") if user.first_name else user.username
    devt = random.choice(zid)
    await edit_or_reply(malath, f"**✾╎نسبـة الكراهيـة لـ** [{devth}](tg://user?id={user.id}) **هـي {devt} 🤮.**")

@blal.dev_cmd(pattern="رفع ورع(?: |$)(.*)")
async def dev(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    if user.id in zel_dev:
        return await edit_or_reply(malath, f"**╮ ❐ عـذراً .. لا يمكننـي اهانـة مطـور السـورس  ❏╰**")
    devth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(malath, f"**🚹 ╎ المستخـدم  ⪼ • ** [{devth2}](tg://user?id={user.id}) \n☑️ **╎تم رفعـه ورع القـروب 😹🙇🏻.**")

@blal.dev_cmd(pattern="رفع مزه(?: |$)(.*)")
async def dev(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    ...
    if user.id in zel_dev:
        return await edit_or_reply(malath, f"**╮ ❐ عـذراً .. لا يمكننـي اهانـة مطـور السـورس  ❏╰**")
    devth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(malath, f"**🚺 ╎ الحلـوه ⪼ • ** [{devth2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـها مـزة الكروب 🥳💃.**")

@blal.dev_cmd(pattern="رفع مطي(?: |$)(.*)")
async def dev(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
@blal.dev_cmd(pattern="رفع حيوان(?: |$)(.*)")
async def permalink(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
async def permalink(malath):
    ...
    devth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(
        malath, f"**🚹 ╎ المستخـدم  ⪼ • ** [{devth2}](tg://user?id={user.id}) \n☑️ **🐈╎ تم رفعـه بـزون .** \n"
    )

@blal.dev_cmd(pattern="رفع زباله(?: |$)(.*)")
async def dev(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
@blal.dev_cmd(pattern="رفع مرتبط(?: |$)(.*)")
async def dev(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
@blal.dev_cmd(pattern="رفع مرتبطه(?: |$)(.*)")
async def dev(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
@blal.dev_cmd(pattern="رفع حبيبي(?: |$)(.*)")
async def dev(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
    devth2 = user.first_name.replace("\u2060", "") if user.first_name else user.username
    await edit_or_reply(malath, f"**🚹 ╎ الحلـو  ⪼ • ** [{devth2}](tg://user?id={user.id}) \n☑️ **╎ تم رفعـه حبيبـج .. بنجـاح 💍🤵‍♂👰🏻‍♀.**")

@blal.dev_cmd(pattern="رفع خطيبتي(?: |$)(.*)")
async def dev(malath):
    user, custom = await get_user_from_event(malath)
    if not user:
        return
...
@blal.dev_cmd(pattern="التحشيش")
async def cmd(Matrixallll):
    await edit_or_reply(Matrixallll, MatrixalTHS_cmd)
