...
from ..helpers.utils import _devutils, reply_id
from . import BOTLOG, BOTLOG_CHATID, mention, blal
LOGS = logging.getLogger(__name__)


@blal.dev_cmd(pattern="احكام(?: |$)(.*)")
async def dev(event): # Code by t.me/zzzzl1l
    user = await event.get_sender()
    userz = blal.uid
    dev_chat = event.chat_id
...
# Code by t.me/zzzzl1l
    dev2 = random.choice(zana2)
    zee2 = random.choice([x for x in zaza2 if x != dev2])
    dev3 = random.choice(zana3)
    zee3 = random.choice([x for x in zaza3 if x != dev3])
    dev4 = random.choice(zana4)
    zee4 = random.choice([x for x in zaza4 if x != dev4])
    dev5 = random.choice(zana5)
    zee5 = random.choice([x for x in zaza5 if x != dev5])
    if gvarstatus("Z_AKM") is None:
    ...
    if gvarstatus("Z_AK") is not None and gvarstatus("Z_A2K") is not None and gvarstatus("Z_A3K") is None and gvarstatus("Z_A4K") is None and gvarstatus("Z_A5K") is None:
        ...
        zilzal = int(zee2)
        try:
            user_dev = await event.client.get_entity(Matrixal)
            user_zee = await event.client.get_entity(zilzal)
        except ValueError:
            return
        name_dev = user_dev.first_name
        name_zee = user_zee.first_name
        await edit_or_reply(event, f"[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗚𝗮𝗺𝗲 - ⚖🧑🏻‍⚖ لعبـة أحكـام](t.me/BDB0B)\n⋆──┄─┄─┄───┄─┄─┄──⋆\n**- تـم اختيـار المتهـم ⇠**  [{name_dev}](tg://user?id={dev2})  \n**- ليتـم الحكـم عليـه ⇠ ⚖**\n**- الحاكـم 👨🏻‍⚖⇠**  [{name_zee}](tg://user?id={zee2}) ", link_preview=False)
        delgvar("Z_AKM")
        return
...
async def dev(event): # Code by t.me/zzzzl1l
    ...
    if gvarstatus("Z_EKB") is None:
        ...
        addgvar("Z_EKB", "true")
        addgvar("Z_EK", userz)
        addgvar("E_CHAT", dev_chat)
        return await edit_or_reply(event, f"[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗚𝗮𝗺𝗲 - ⚖🧑🏻‍⚖ لعبـة عقـاب](t.me/BDB0B)\n⋆──┄─┄─┄───┄─┄─┄──⋆\n**- تم بـدء اللعبـة وتـم إنضمـامي**  [{user.first_name}](tg://user?id={user.id})  **بنجـاح ☑️**\n\n**- اللي بيلعـب يرسل**  `.انا` ", link_preview=False)
    else:
        delgvar("Z_EKM")
        delgvar("Z_EK")
        delgvar("Z_E2K")
...
@blal.dev_cmd(pattern="(ايقاف عقاب|انهاء عقاب)$")
async def dev(event): # Code by t.me/zzzzl1l
    if gvarstatus("Z_EKB") is None:
        delgvar("Z_EKM")
        delgvar("Z_EK")
...
async def _(event): # Code by t.me/zzzzl1l
    ...
    if gvarstatus("Z_EKB") is not None and event.chat_id == int(gvarstatus("E_CHAT")):
        if user.id == blal.uid:
            return await event.reply(f"[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗚𝗮𝗺𝗲 - ⚖🧑🏻‍⚖ لعبـة عقـاب](t.me/BDB0B)\n⋆──┄─┄─┄───┄─┄─┄──⋆\n**- انت منضم مسبقـاً ؟!**", link_preview=False)
        if gvarstatus("Z_E2K") is None:
            ...
            try:
                ...
                u1 = await blal(GetUsersRequest(zzz1))
            ...
            try:
                ...
                u2 = await blal(GetUsersRequest(zzz2))
            zillzall = f"[{user.first_name}](tg://user?id={user.id})"
            zilzal1 = f"[{u1.first_name}](tg://user?id={u1.id})"
            ...
            try:
                ...
                u3 = await blal(GetUsersRequest(zzz3))
            zillzall = f"[{user.first_name}](tg://user?id={user.id})"
            zilzal1 = f"[{u1.first_name}](tg://user?id={u1.id})"
            zilzal2 = f"[{u2.first_name}](tg://user?id={u2.id})"
            ...
            try:
                ...
                u4 = await blal(GetUsersRequest(zzz4))
            zillzall = f"[{user.first_name}](tg://user?id={user.id})"
            zilzal1 = f"[{u1.first_name}](tg://user?id={u1.id})"
            zilzal2 = f"[{u2.first_name}](tg://user?id={u2.id})"
            zilzal3 = f"[{u3.first_name}](tg://user?id={u3.id})"
            ...
            try:
                ...
                u5 = await blal(GetUsersRequest(zzz5))
            zillzall = f"[{user.first_name}](tg://user?id={user.id})"
            zilzal1 = f"[{u1.first_name}](tg://user?id={u1.id})"
            zilzal2 = f"[{u2.first_name}](tg://user?id={u2.id})"
            zilzal3 = f"[{u3.first_name}](tg://user?id={u3.id})"
...
# Code by t.me/zzzzl1l
    ...
    dev5 = random.choice(zana5)
    zee5 = random.choice([x for x in zaza5 if x != dev5])
    dev6 = random.choice(zana6)
    zee6 = random.choice([x for x in zaza6 if x != dev6])
    if gvarstatus("Z_EKB") is None:
    ...
    if gvarstatus("Z_EK") is not None and gvarstatus("Z_E2K") is not None and gvarstatus("Z_E3K") is None and gvarstatus("Z_E4K") is None and gvarstatus("Z_E5K") is None and gvarstatus("Z_E6K") is None:
        ...
        try:
            ...
            devph = [message for message in messages]
            aing = await event.client.get_me()
            chosen_photo = random.sample(devph, 1)[0]
            await event.client.send_file(
                event.chat_id,
                file=chosen_photo,
                caption=f"[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗚𝗮𝗺𝗲 - ⚖⛓ لعبـة عقـاب](t.me/BDB0B)\n⋆──┄─┄─┄───┄─┄─┄──⋆\n**- تم تحديـد المجـرم 🥷 ⇠**  [{name_dev}](tg://user?id={dev2})  \n**- الجـلاد 👨🏻‍⚖⇠**  [{name_zee}](tg://user?id={zee2}) \n**- العقاب : تغيير صورة حسابك بالصورة اعلاه والاسم والبايو يحددهم الجلاد ⚖**",
            )
            await event.delete()
            delgvar("Z_EKB")
            return
    ...
    if gvarstatus("Z_EK") is not None and gvarstatus("Z_E2K") is not None and gvarstatus("Z_E3K") is not None and gvarstatus("Z_E4K") is None and gvarstatus("Z_E5K") is None and gvarstatus("Z_E6K") is None:
        ...
        try:
            ...
            await event.client.send_file(
                event.chat_id,
                file=chosen_photo,
                caption=f"[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗚𝗮𝗺𝗲 - ⚖⛓ لعبـة عقـاب](t.me/BDB0B)\n⋆──┄─┄─┄───┄─┄─┄──⋆\n**- تم تحديـد المجـرم 🥷 ⇠**  [{name_dev}](tg://user?id={dev3})  \n**- الجـلاد 👨🏻‍⚖⇠**  [{name_zee}](tg://user?id={zee3}) \n**- العقاب : تغيير صورة حسابك بالصورة اعلاه والاسم والبايو يحددهم الجلاد ⚖**",
            )
            await event.delete()
            delgvar("Z_EKB")
            return
    ...
    if gvarstatus("Z_EK") is not None and gvarstatus("Z_E2K") is not None and gvarstatus("Z_E3K") is not None and gvarstatus("Z_E4K") is not None and gvarstatus("Z_E5K") is None and gvarstatus("Z_E6K") is None:
        ...
        try:
            ...
            await event.client.send_file(
                event.chat_id,
                file=chosen_photo,
                caption=f"[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗚𝗮𝗺𝗲 - ⚖⛓ لعبـة عقـاب](t.me/BDB0B)\n⋆──┄─┄─┄───┄─┄─┄──⋆\n**- تم تحديـد المجـرم 🥷 ⇠**  [{name_dev}](tg://user?id={dev4})  \n**- الجـلاد 👨🏻‍⚖⇠**  [{name_zee}](tg://user?id={zee4}) \n**- العقاب : تغيير صورة حسابك بالصورة اعلاه والاسم والبايو يحددهم الجلاد ⚖**",
            )
            await event.delete()
            delgvar("Z_EKB")
            return
    ...
    if gvarstatus("Z_EK") is not None and gvarstatus("Z_E2K") is not None and gvarstatus("Z_E3K") is not None and gvarstatus("Z_E4K") is not None and gvarstatus("Z_E5K") is not None and gvarstatus("Z_E6K") is None:
        ...
        try:
            ...
            await event.client.send_file(
                event.chat_id,
                file=chosen_photo,
                caption=f"[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗚𝗮𝗺𝗲 - ⚖⛓ لعبـة عقـاب](t.me/BDB0B)\n⋆──┄─┄─┄───┄─┄─┄──⋆\n**- تم تحديـد المجـرم 🥷 ⇠**  [{name_dev}](tg://user?id={dev5})  \n**- الجـلاد 👨🏻‍⚖⇠**  [{name_zee}](tg://user?id={zee5}) \n**- العقاب : تغيير صورة حسابك بالصورة اعلاه والاسم والبايو يحددهم الجلاد ⚖**",
            )
            await event.delete()
            delgvar("Z_EKB")
            return
...
@blal.dev_cmd(pattern="ذاتي (\\d*) ([\\s\\S]*)")
...
@blal.dev_cmd(pattern="ذااتي (\\d*) ([\\s\\S]*)")
async def selfdestruct(destroy):
    zzz = ("".join(destroy.text.split(maxsplit=1)[1:])).split(" ", 1)
    message = zzz[1]
    ttl = int(zzz[0])
...
@blal.on(admin_cmd(pattern="(خط الغامق|خط غامق|تفعيل غامق|تفعيل الغامق)"))
async def _(event):
...
@blal.on(admin_cmd(pattern="(تعطيل غامق|تعطيل الغامق)"))
async def _(event):
    is_thin = gvarstatus("thin")
    if is_thin:
        delgvar("thin")
...
@blal.on(admin_cmd(pattern="(خط المائل|خط مائل|تفعيل مائل|تفعيل المائل)"))
async def _(event):
...
@blal.on(admin_cmd(pattern="(تعطيل مائل|تعطيل المائل)"))
async def _(event):
    is_mael = gvarstatus("mael")
    if is_mael:
        delgvar("mael")
...
@blal.on(admin_cmd(pattern="(خط التشويش|خط تشويش|تفعيل تشويش|تفعيل التشويش)"))
async def _(event):
...
@blal.on(admin_cmd(pattern="(تعطيل تشويش|تعطيل التشويش)"))
async def _(event):
    is_cllear = gvarstatus("cllear")
    if is_cllear:
        delgvar("cllear")
...
@blal.on(admin_cmd(pattern="(خط النسخ|خط نسخ|تفعيل نسخ|تفعيل النسخ)"))
async def _(event):
...
@blal.on(admin_cmd(pattern="(تعطيل نسخ|تعطيل النسخ)"))
async def _(event):
    is_cood = gvarstatus("cood")
    if is_cood:
        delgvar("cood")
...
async def Matrixal_gif(event):
    ...
    zz = "ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - زخرفـه انكلـش\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆"
    aa = f"`{s2}`\n`{s3}`\n`{s4}`\n`{s5}`\n`{s6}`\n`{s7}`\n`{s8}`\n`{s9}`\n`{s10}`\n`{s11}`\n`{s12}`\n`{s13}`\n`{s14}`\n`{s15}`\n`{s16}`\n`{s17}`\n`{s18}`\n`{s19}`\n`{s20}`\n`{s21}`\n`{s22}`\n`{s23}`\n`{s24}`\n`{s25}`\n`{s26}`\n`{s27}`\n`{s28}`\n`{s29}`\n`{s30}`\n`{s31}`"
