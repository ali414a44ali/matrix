...
'''
mm = '''
**- عليك الانضمـام في قنـاة السـورس اولاً**  @BDB0B
'''
...
keyboard = [
  ...
  [
    Button.url("𝙈𝙖𝙏𝙍𝙞𝙭 ⌁™ 𓅛", "https://t.me/BDB0B")
    ]
]
...
@blal.dev_cmd(pattern="الطقس(?: |$)(.*)")
async def _(event):
    await edit_or_reply(event, "**- ارسـل .طقس + اسـم المدينـة**\n\n**- مثــال :**\n.طقس بغداد")


@blal.dev_cmd(pattern="طقس (.*)")
async def _(event):
    if event.fwd_from:
        return
    dev = "adf0cf22618186fc11e9f89c090cb356"
    sample_url = "https://api.openweathermap.org/data/2.5/weather?q={}&appid={}"
    input_str = event.pattern_match.group(1)
    async with aiohttp.ClientSession() as session:
        response_api_zero = await session.get(sample_url.format(input_str, dev))
...
@blal.dev_cmd(pattern="وهمي كتابه(?: |$)(.*)")
async def _(event):
...
@blal.dev_cmd(pattern="وهمي فيديو(?: |$)(.*)")
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@blal.dev_cmd(pattern="وهمي لعبه(?: |$)(.*)")
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@blal.dev_cmd(pattern="وهمي صوره(?: |$)(.*)")
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@blal.dev_cmd(pattern="وهمي موقع(?: |$)(.*)")
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@blal.dev_cmd(pattern="وهمي جهات(?: |$)(.*)")
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@blal.dev_cmd(pattern="وهمي مرئي(?: |$)(.*)")
async def _(event):
    t = event.pattern_match.group(1)
    if not (t or t.isdigit()):
        t = 100
...
@blal.dev_cmd(pattern="ايقاف الوهمي(?: |$)(.*)")
async def _(event):
