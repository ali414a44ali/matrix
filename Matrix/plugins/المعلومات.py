...
from Matrix import StartTime, blal, tepversion
...
@blal.dev_cmd(pattern="الاضافات$")
async def _(event):
    cmd = "ls Matrix/plugins"
...
async def devdd(event):
    ...
    devt = "**- تاريخ تنصيبـك لـ بـوت تيبثـــون - 𓆩𝙎𝙊𝙐𝙍𝘾𝞝 𝙏𝞝𝙋 𓆪**\n\n"
    uptime = await get_readable_time((time.time() - StartTime))
    boot_time_timestamp = psutil.boot_time()
    bt = datetime.fromtimestamp(boot_time_timestamp)
    devt += f"**- التاريـخ 📋:**\t`{bt.day}/{bt.month}/{bt.year}`\n**- الـوقت ⏰:**\t`{bt.hour}:{bt.minute}`\n**- المـدة المنقضيـه ⏳:**\t`{uptime}`\n"
