...
@blal.dev_cmd(pattern="همسه(?: |$)(.*)")
async def repoblal(event):
    global bbb
    ...
    response = await blal.inline_query(Config.TG_BOT_USERNAME, "zelzal")
    await response[0].click(event.chat_id)
    await event.delete()

@blal.zed_cmd(pattern="اهمس(?: |$)(.*)")
async def repoblal(event):
    global bbb
    if gvarstatus("dev_Vip") is None and dev_Uid not in dev_Dev:
        return await edit_or_reply(event, "**⎉╎عـذࢪاً .. ؏ـزيـزي\n⎉╎هـذا الامـر ليـس مجـانـي📵\n⎉╎للاشتـراك في الاوامـر المدفوعـة\n⎉╎تواصـل مطـور السـورس **")
