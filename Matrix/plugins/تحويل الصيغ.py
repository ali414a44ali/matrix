...
from ..helpers.utils import _devtools, _devutils, _format
from . import blal, make_gif, progress
...
async def pic_gifcmd(event):  # sourcery no-metrics
    ...
    output = await _devtools.media_to_pic(event, reply)
    if output[1] is None:
...
@blal.dev_cmd(pattern="ملصق متحرك(?: |$)(.*)")
...
@blal.dev_cmd(pattern="ممتحركة(?: |$)((-)?(r|l|u|d|s|i)?)$")
async def pic_gifcmd(event):
    ...
    imag = await _devtools.media_to_pic(event, reply)
    if imag[1] is None:
        return await edit_delete(
...
@blal.dev_cmd(pattern="متحرك ?([0-9.]+)?$")
async def _(event):
    reply = await event.get_reply_message()
