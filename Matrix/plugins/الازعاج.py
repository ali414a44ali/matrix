...
@blal.dev_cmd(pattern="(تقليد|ازعاج)$")
async def echo(event):
    if event.reply_to_msg_id is None:
        return await edit_or_reply(event, "**- بالـرد على الشخص الذي تريد ازعاجـه**")
    blalevent = await edit_or_reply(event, "**- يتم تفعيل هذا الامر انتظر قليلا ..**")
    user, rank = await get_user_from_event(event, blalevent, nogroup=True)
    if not user:
        return
    ...
    try:
        ...
        await edit_delete(blalevent, f"**⎉╎خطـأ :**\n`{str(e)}`")
    else:
        await edit_or_reply(
            blalevent,
...
@blal.dev_cmd(pattern="(الغاء تقليد|ايقاف التقليد|ايقاف تقليد|الغاء الازعاج|ايقاف الازعاج|ايقاف ازعاج|الغاء ازعاج)$")
async def echo(event):
    if event.reply_to_msg_id is None:
...
@blal.dev_cmd(pattern="حذف المقلدهم( للكل)?")
async def echo(event):
...
@blal.dev_cmd(pattern="المقلدهم( للكل)?$")
async def echo(event):
    input_str = event.pattern_match.group(1)
    private_chats = ""
...
@blal.dev_cmd(incoming=True, edited=False)
