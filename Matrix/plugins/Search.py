...
from Matrix.utils import sudo_cmd
...
async def download_video(event):
    ...
    if not url and rmsg:
        ...
        url = re.search("(?P<url>https?://[^\s]+)", myString).group("url")
    if not url:
        return await edit_or_reply(event, "᯽︙ عـليك وضع رابـط أولًا ليتـم تنـزيله")
    catevent = await edit_or_reply(event, "᯽︙ يتم التحميل انتظر قليلًا")
    reply_to_id = await reply_id(event)
...
async def yt_search(event):
    ...
    if not query:
        return await edit_delete(
            event, "**᯽︙ قم بالرد على النص او كتابته مع الأمر**"
        )
...
@blal.on(admin_cmd(pattern="تيك توك(?: |$)(.*)"))
async def _(event):
    if event.fwd_from:
        return
    r_link = event.pattern_match.group(1)
    if ".com" not in r_link:
        await event.edit("**▾∮ يجب وضع رابط الفيديو مع الأمر أولًا **")
    else:
        await event.edit("**▾∮ تتم المعالجة انتظر قليلًا**")
    chat = "@ttsavebot"
