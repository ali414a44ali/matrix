...
@blal.dev_cmd(pattern="(مغادرة الكروبات|مغادرة المجموعات)")
async def leave_groups(event):
    ...
    try:
        ...
        for group in gg:
            if group not in ss:
                await blal.delete_dialog(group)
                num += 1
                await sleep(1)
        if num >=1:
...
@blal.dev_cmd(pattern="مغادرة القنوات")
...
@blal.dev_cmd(pattern="حذف الخاص")
async def _(event):
...
@blal.dev_cmd(pattern="حذف البوتات")
async def _(event):
...
@blal.dev_cmd(pattern="تفليش بالبوت$", groups_only=True)
...
@blal.dev_cmd(pattern=f"{HDRALL}$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@blal.dev_cmd(pattern=f"{KTMALL}$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@blal.dev_cmd(pattern="الغاء التفليش", groups_only=True)
