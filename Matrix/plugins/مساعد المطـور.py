...
ZelzalDV_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 - اوامــر المطـور المســاعد](t.me/Matrix) 𓆪\n\n"
...
async def add_sudo_user(event):
    ...
    output += "**✾╎يتم الان اعـادة تشغيـل بـوت تيبثـون 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 :**\n\n"
...
@blal.dev_cmd(pattern="حذف_المطورين")
async def _(event):
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "آمن":
        devevent = await edit_or_reply(event, "**✾╎تـم تفعيـل التحكـم للمطـوريـن لـ الاوامـر الآمـنـه .. بنجـاح🧑🏻‍💻✅**")
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += (
                    f"`{plugin}` __There is no such plugin in your Matrix__.\n"
                )
            else:
    ...
    output = (
        result + "**✾╎يتم الان اعـادة تشغيـل بـوت تيبثـون 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
    msg = await edit_or_reply(devevent, output)
    await event.client.reload(msg)
...
blal.loop.create_task(_init())



# Copyright (C) 2022 dev-Thon . All Rights Reserved
@blal.dev_cmd(pattern="المساعد")
