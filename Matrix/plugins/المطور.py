...
MatrixalDV_cmd = (
    "[ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - اوامــر المطـور المســاعد](t.me/BDB0B) .\n\n"
...
async def add_sudo_user(event):
    ...
    output += "**⎉╎يتم الان اعـادة تشغيـل بـوت ماتركس انتظـر 2-1 دقيقـه ▬▭ ...**"
...
async def _(event):
    ...
    result = "**•❐• قائمــة مـطـورين البــوت الخـاص بـك مـن 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    output = (
        result + "**⎉╎يتم الان اعـادة تشغيـل بـوت ماتركس انتظـر 2-1 دقيقـه ▬▭ ...**\n"
    )
    if errors != "":
        output += "\n**- خطــأ :**\n" + errors
...
async def cmd(Matrixallll):
    await edit_or_reply(Matrixallll, MatrixalDV_cmd)
