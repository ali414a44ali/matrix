...
async def tagall(event):
  ...
  if mode == "by_cmd":
    ...
    usrnum = 0
    usrtxt = ""
    async for usr in blal.iter_participants(event.chat_id):
      usrnum += 1
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await blal.send_message(event.chat_id, f"{usrtxt}\n\n- {msg}")
  ...
  if mode == "by_reply":
    ...
    async for usr in blal.iter_participants(event.chat_id):
      ...
      if event.chat_id not in moment_worker:
        ...
        return
      if usrnum == 5:
        await blal.send_message(event.chat_id, usrtxt, reply_to=msg)
        await asyncio.sleep(2)
        usrnum = 0
        usrtxt = ""
...
@blal.dev_cmd(pattern="ايقاف التاك?(.*)")
...
@blal.dev_cmd(pattern="تاك(?:\s|$)([\s\S]*)")
async def tagall(event):
  global moment_worker
  if event.is_private:
    return await edit_or_reply(event, "**- عـذراً ... هـذه ليـست مجمـوعـة ؟!**")
