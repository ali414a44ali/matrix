...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("مساعدة") and event.query.user_id == blal.uid:
            ...
            #if gvarstatus("dev_Vip") is None or dev_Uid not in dev_dev:
                #return
...
async def who(event):
    ...
    if input_str or reply:
        return await edit_or_reply(event, "**- ارسـل الامـر بـدون رد**")
    if (event.chat_id in dev_BLACKLIST) and (dev_Uid not in dev_dev):
...
@check_owner
async def _(event):
    await event.edit(matrixalTZ_cmd, buttons=[[Button.inline("رجــوع", data="devHELP")]], link_preview=False)
...
**- مثـال :**
`.اختصار https://github.com/dev-Thon/matrixal`
