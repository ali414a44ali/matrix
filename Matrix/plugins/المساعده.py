...
zelzal = (7645521594, 7645521594, 7645521594)
...
async def get_user_from_event(event):
    if event.reply_to_msg_id:
        ...
        if isinstance(user, int) or user.startswith("@"):
            ...
            return user_obj
        try:
            user_object = await blal.get_entity(user)
...
# Copyright (C) 2023 t.me/BDB0B . All Rights Reserved
...
async def fetch_info(event):
    ...
    replied_user = await blal.get_me()
    #user = self_user.id
    FullUser = (await blal(GetFullUserRequest(replied_user.id))).full_user
    replied_user_profile_photos = await blal(
    ...
    zilzal = (await blal.get_entity(user_id)).premium
    if zilzal == True or user_id in zelzal: #Code by T.me/zzzzl1l
        zpre = "ℙℝ𝔼𝕄𝕀𝕌𝕄 🌟"
    else:
        zpre = "𝕍𝕀ℝ𝕋𝕌𝔸𝕃 ✨"
    #zid = int(gvarstatus("dev_Vip"))
    ...
    photo = await blal.download_profile_photo(
        user_id,
        photo_path,
        download_big=True,
    )
...
################# Dev ZilZal #################
    if user_id in zelzal: #Code by T.me/zzzzl1l
        ...
        rotbat = "مـطـور 𐏕" 
    elif user_id == (await blal.get_me()).id:
        rotbat = "مـالك الحساب 𓀫" 
    else:
        rotbat = "العضـو 𓅫"
################# Dev ZilZal #################
    dev_TEXT = gvarstatus("CUSTOM_ALIVE_TEXT") or "•⎚• مـعلومـات المسـتخـدم مـن بـوت ماتركـس"  #Code by T.me/zzzzl1l
    devM = gvarstatus("CUSTOM_ALIVE_EMOJI") or "✦ " #Code by T.me/zzzzl1l
    devF = gvarstatus("CUSTOM_ALIVE_FONT") or "⋆─┄─┄─┄─ ᴹᵃᵗʳⁱˣ ─┄─┄─┄─⋆" #Code by T.me/zzzzl1l
    if gvarstatus("ZID_TEMPLATE") is None:
        caption = f"<b> {dev_TEXT} </b>\n"
        caption += f"ٴ<b>{devF}</b>\n"
        caption += f"<b>{devM}الاســم    ⤎ </b> "
        ...
        caption += f"<b>{devM}التفاعل  ⤎</b>  {zelzzz}\n" #Code by T.me/zzzzl1l
        if user_id != (await blal.get_me()).id: #Code by T.me/zzzzl1l
            caption += f"<b>{devM}الـمجموعات المشتـركة ⤎  {common_chat}</b>\n"
        caption += f"<b>{devM}الإنشـاء  ⤎</b>  {zzzsinc}  🗓\n" #Code by T.me/zzzzl1l
        caption += f"<b>{devM}البايـو     ⤎  {user_bio}</b>\n"
        caption += f"ٴ<b>{devF}</b>"
    else:
...
ZelzalTZ_cmd = (
    ...
    "𓆩 [𝙈𝙖𝙏𝙍𝙞𝙭  𝗩𝗮𝗿𝘀 - قنـاة الفـارات](t.me/BDB0B) 𓆪"
)

dev_temp = """
┏───────────────┓
│ ◉ sᴏʀᴄᴇ ᴹᵃᵗʳⁱˣ ɪs ʀᴜɴɴɪɴɢ ɴᴏᴡ
┣───────────────┫
│ ● ɴᴀᴍᴇ ➪  {mention}
│ ● ᴹᵃᵗʳⁱˣ ➪ {telever}
│ ● ᴘʏᴛʜᴏɴ ➪ {pyver}
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        query = event.text
        await blal.get_me()
        if query.startswith("مساعدة") and event.query.user_id == blal.uid:
            buttons = [
                ...
                [
                    Button.inline("التسليـه & التحشيش 🎃", data="fundev"),
                ],
                [
                    Button.inline("المرفقـات 🪁", data="extras"),
                    Button.inline("الادوات 💡", data="tooldev"),
                ],
                [
                    Button.inline("الفـارات 🎈", data="varsdev"),
                ],
                [
                    Button.inline("الذكـاء الاصطنـاعـي 🛸", data="devAI"),
                ],
                [
        ...
        elif query.startswith("الفحص") and event.query.user_id == blal.uid:
            ...
            USERID = blal.uid if Config.OWNER_ID == 0 else Config.OWNER_ID
            ...
            await event.answer([result] if result else None)
        elif query.startswith("idid") and event.query.user_id == blal.uid:
            #if gvarstatus("dev_Vip") is None or Zel_Uid not in dev_dev:
                #return
            ...
            try:
                ...
                return await edit_or_reply(dev, "**- لـم استطـع العثــور ع الشخــص ؟!**")
...
async def who(event):
    if gvarstatus("dev_Vip") is None and Zel_Uid not in dev_dev:
    ...
    if event.reply_to_msg_id:
        ...
        return
    response = await blal.inline_query(Config.TG_BOT_USERNAME, "idid")
...
@blal.dev_cmd(pattern="مساعده")
async def help(event):
    if event.reply_to_msg_id:
        await event.get_reply_message()
    response = await blal.inline_query(Config.TG_BOT_USERNAME, "مساعدة")
    await response[0].click(event.chat_id)
    await event.delete()
...
async def help(event):
    if event.reply_to_msg_id:
        await event.get_reply_message()
    response = await blal.inline_query(Config.TG_BOT_USERNAME, "الفحص")
    await response[0].click(event.chat_id)
    await event.delete()
...
async def on_all_liked_delete(event):
    ...
    count = 1
    if liikers:
        dev = await edit_or_reply(event, "**⪼ جـارِ مسـح المعجبيـن .. انتظـر ⏳**")
        for mogab in liikers:
            count += 1
        remove_all_likes(blal.uid)
        delgvar("Like_Id")
        await dev.edit("**⪼ تم حـذف جميـع المعجبيـن .. بنجـاح ✅**")
    else:
        OUT_STR = "**- مسكيـن ع باب الله 🧑🏻‍🦯**\n**- ماعنـدك معجبيـن حالياً ❤️‍🩹**"
        await edit_or_reply(event, OUT_STR)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"likes")))
async def _(event):
    # الحصول على معلومات الشخص الذي قام بالضغط على الزر
    user_id = event.sender_id
    try:
        user = await blal.get_entity(user_id)
        user_name = f"{user.first_name}{user.last_name}" if user.last_name else user.first_name
        user_username = f"@{user.username}" if user.username else "لا يوجد"
    except ValueError:
        user = await blal(GetUsersRequest(user_id))
        user_name = f"{user.first_name}{user.last_name}" if user.last_name else user.first_name
        user_username = f"@{user.username}" if user.username else "لا يوجد"
    except Exception:
    ...
    if add_like(str(blal.uid), str(user.id), user_name, user_username) is True:
        Like_id += 1
        addgvar("Like_Id", Like_id)
    else:
        return await event.answer("- انت معجب من قبل بهذا الشخص ❤️", cache_time=0, alert=True)
        #remove_like(str(blal.uid), str(user.id))
        #if add_like(str(blal.uid), str(user.id), user_name, user_username) is True:
            #return await
    try:
        ...
        await blal.send_message(BOTLOG_CHATID, f"**- حدث خطأ أثناء إرسال إشعـار لايك ❌**\n**- الخطأ هـو 📑:**\n-{e}")
    # تحديث زر الإعجاب
    try:
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"devHELP")))
@check_owner
async def _(event):
    butze = [
        [Button.inline("البـحـث والتحميـل 🪄", data="zdownload")],
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"kalaysh")))
@check_owner
async def _(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر تخصيص الكلايـش 🪁](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي قنـوات تخصيص كلايـش السـورس**\n**⎉╎القنوات تحتوي على كلايش متنوعه + اوامر اضافة الكلايش**",
        buttons=[
    ...
    try:
        await event.edit(
            ...
            buttons=[
                ...
                [Button.inline("رجوع", data="devHELP")],
            ],
        link_preview=False)
    except Exception:
        await event.answer(zelzal, cache_time=0, alert=True)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"zzcall")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المكالمـات والميـوزك 🎸](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"zzmusic")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المكالمـات والميـوزك 🎸](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"superzzz")))
@check_owner
async def _(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - النشـر التڪـراري العـام (الـسوبـرات) 🎡](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر النشـر والسوبـرات :**\n\n",
        buttons=[
            [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"spamzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر النشــر التلقــائي 🌐](t.me/BDB0B  ) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"superrzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭  🎡 النشـࢪ التڪࢪاࢪي](t.me/ZTho) .
**⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆**
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"pointzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر تجميــع النقــاط 🛂](t.me/BDB0B) .
...
async def _(event):
    await event.edit(
        ...
        buttons=[
            ...
            [Button.inline("رجـوع", data="devHELP")],
        ],
    link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"zchatgpt3")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗖𝗵𝗮𝘁𝗚𝗽𝘁 - الذكـاء الاصطناعـي 🧠](t.me/BDB0B) .
**⋆┄─┄─┄─┄─┄─┄─┄─┄─┄⋆**
**- الامـر :**
**⪼** `.س` + سـؤالك
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"ztgpt")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗭𝗧𝗚𝗽𝘁 - الذكـاء الاصطناعـي 🧠](t.me/BDB0B) .
**⋆┄─┄─┄─┄─┄─┄─┄─┄─┄⋆**
**- الامـر :**
**⪼** `.زد` + سـؤالك
...
.زد من هو مكتشف الجاذبية الارضية""",
        buttons=[
            [Button.inline("رجوع", data="devAI")],
        ],
    link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"zgemini")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗚𝗲𝗺𝗶𝗻𝗶.𝗔𝗶 - الذكـاء الاصطناعـي 🧠](t.me/BDB0B) .
**⋆┄─┄─┄─┄─┄─┄─┄─┄─┄⋆**
**- الامـر :**
**⪼** `.جيمي` + سـؤالك
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"zphotoai")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗣𝗵𝗼𝘁𝗼.𝗔𝗶 - الذكـاء الاصطناعـي 🧠](t.me/BDB0B) .
**⋆┄─┄─┄─┄─┄─┄─┄─┄─┄⋆**
**- الامـر :**
**⪼** `.ارسم` + طلبـك
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"zdarkai")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗗𝗮𝗿𝗸.𝗔𝗶 - الذكـاء الاصطناعـي 🧠](t.me/BDB0B) .
**⋆┄─┄─┄─┄─┄─┄─┄─┄─┄⋆**
**- الامـر :**
**⪼** `.دارك` + سـؤالك
...
############ البوت ############
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"botvr")))
@check_owner
async def _(event):
    zelzal = "⤶ عـذراً عـزيـزي 🤷🏻‍♀\n⤶ هـذه اللوحه لا تشتغل في الخاص\n⤶ لـ إظهـار لوحـة المسـاعـدة 👇\n\n⤶ ارســل (.مساعده) في اي مجمـوعـه"
    try:
        await event.edit(
            "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر البـوت المسـاعد :**\n\n",
            buttons=[
                [
                ...
                [
                    Button.inline("الفحص", data="alivdev"),
                ],
                [
                    Button.inline("السليب (النوم)", data="sleep"),
                    Button.inline("سرعة الانترنت", data="netdev"),
                ],
                [
                    Button.inline("نظـام البـوت", data="sysdev"),
                ],
                [
                    Button.inline("سورس", data="sourcevr"),
                    Button.inline("ماتركـس", data="devvr"),
                ],
                [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"sysdev")))
@check_owner
async def _(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - ادوات النظــام 🤖](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر نظـام البـوت المسـاعد :**\n\n",
        buttons=[
            [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"syszzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
ارسـل الامـر   `.النظام`""",
        buttons=[
            [Button.inline("رجوع", data="sysdev")],
        ],
    link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"rmzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"fszzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"envzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"datzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"updatevr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"resitvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"stopvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"alivdev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"sleep")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"netdev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"sourcevr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  ??𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
**- الامـر :**
**⪼** `.سورس`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"devvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
**- الاستخـدام :**
ارسـل الامـر   `.ماتركـس`""",
        buttons=[
            [Button.inline("رجوع", data="botvr")],
        ],
    link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"ethaavr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الســورس 🌐](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"warndev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
**- الامـر :**
**⪼** `.تحذير`
**⪼** `.التحذيرات`
**⪼** `.حذف فار التحذيرات`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"group4vr")))
@check_owner
async def _(event):
    await event.edit(
        """**- الامـر :**
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"group5vr")))
@check_owner
async def _(event):
    await event.edit(
        """**- الامـر :**
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"grouppro")))
@check_owner
async def _(event):
    await event.edit(
        ...
        buttons=[
            ...
            [Button.inline("تقييـد المحتـوى", data="loldev")],
            [Button.inline("رجوع", data="groupvr")],
        ],
    link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"botveiw")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"botlock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"addlock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿??𝗼?? - اوامــر المجمــوعــه¹](t.me/BDB0B) .
**- الامـر :**
**⪼** `.قفل الاضافه`
`.فتح الاضافه`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"golock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"medlock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"edmdlock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"urlock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"userlock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"forlock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"inilock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀??𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
**- الامـر :**
**⪼** `.قفل الانلاين`
`.فتح الانلاين`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"farslock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"fuklock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"premlock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"zerolock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"alllock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"setelock")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه¹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"loldev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"groupv1")))
@check_owner
async def _(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه² 🛗](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر المجمــوعــه² :**\n\n",
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"urlveiw")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"tagvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"addmnvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"creatorvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"msgvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"delmsgvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲??𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
**- الامـر :**
**⪼** `.حذف فار رسائلي`
**⪼** `.مسح`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"iundlt")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"infoovr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"memver")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"creatorrvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"botssvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"photoadd")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"pinvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"zomby")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"delbans")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه²](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"groupv2")))
@check_owner
async def _(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³ 🛗](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر المجمــوعــه³ :**\n\n",
        buttons=[
            ...
            [
                Button.inline("التحذيرات", data="warndev"),
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"mutevr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"banvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"kickvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"tkkkvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"nospam")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر مكـافح التكــرار 🛡](t.me/BDB0B) .
**- الامـر :**
**⪼** `.ضع التكرار`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"noway")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المنــع 🚫](t.me/BDB0B) .
**- الامـر :**
**⪼** `.منع`
**⪼** `.الغاء منع`
**⪼** `.قائمة المنع`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"group0vr")))
@check_owner
async def _(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الاضـافه والتفليـش 👾](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر الاضـافه والتفليـش :**\n\n",
        buttons=[
            [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"addvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"zerovr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"banall")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"kickall")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"mutall")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"byby")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"banbot")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"zoomby")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"dellbans")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المجمــوعــه³](t.me/BDB0B) .
...
############ الفارات ############
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"varsdev")))
@check_owner
async def _(event):
    zelzal = "⤶ عـذراً عـزيـزي 🤷🏻‍♀\n⤶ هـذه اللوحه لا تشتغل في الخاص\n⤶ لـ إظهـار لوحـة المسـاعـدة 👇\n\n⤶ ارســل (.مساعده) في اي مجمـوعـه"
    try:
        await event.edit(
            "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات 🧬](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر الفــارات :**\n\n",
            buttons=[
                [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"namevar")))
@check_owner
async def _(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر فــارات الوقتــي 🕰](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر فــارات الوقتــي :**\n\n",
        buttons=[
            [
            ...
            [Button.inline("رجوع", data="varsdev")],
            [Button.inline("رجــوع", data="devHELP")],
        ],
    link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"contrytime")))
@check_owner
async def _(event):
    await event.edit(ZelzalTZ_cmd, buttons=[[Button.inline("رجــوع", data="devHELP")]], link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"symnamvar")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"phovarlok")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"biolokvar")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"timevar")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"nameprvr")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"pmvars")))
@check_owner
async def _(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر فــارات حمايـة الخــاص 🛄](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر فــارات حمايـة الخــاص :**\n\n",
        buttons=[
            [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"warnvars")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"pmvarkish")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"picpmvar")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"alivevar")))
@check_owner
async def _(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر فــارات الفحـص 🏮](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر فــارات الفحـص :**\n\n",
        buttons=[
            [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"picvars")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"kleshalive")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"rmzalive")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"sourcevar")))
@check_owner
async def dev_help(event):
    zelzal = "⤶ عـذراً عـزيـزي 🤷🏻‍♀\n⤶ هـذه اللوحه لا تشتغل في الخاص\n⤶ لـ إظهـار لوحـة المسـاعـدة 👇\n\n⤶ ارســل (.مساعده) في اي مجمـوعـه"
    try:
        await event.edit(
            "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - فـارات السـورس 🛰](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات بعـض اوامـر فـارات السـورس :**\n\n",
            buttons=[
                [
                ...
                [Button.inline("رجــوع", data="varsdev")],
            ],
        link_preview=False)
    except Exception:
        await event.answer(zelzal, cache_time=0, alert=True)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"katmvar")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼?? - اوامــر الفــارات](t.me/BDB0B) .
**- الامـر :**
**⪼** `.اضف فار صورة الكتم`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"startbotvar")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"timavar")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
https://t.me/dev_Thon/148""",
        buttons=[
            [Button.inline("رجوع", data="namevar")],
        ],
    link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"pointvar")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الفــارات](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"zdownload")))
@check_owner
async def dev_help(event):
    zelzal = "⤶ عـذراً عـزيـزي 🤷🏻‍♀\n⤶ هـذه اللوحه لا تشتغل في الخاص\n⤶ لـ إظهـار لوحـة المسـاعـدة 👇\n\n⤶ ارســل (.مساعده) في اي مجمـوعـه"
    try:
        await event.edit(
            "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر البحث والتحميـل من جميـع مواقـع الـ سوشـل ميديـا :**\n\n",
            buttons=[
                [
                    Button.inline("فيديو", data="veddev"),
                    Button.inline("بحث", data="songdev"),
                ],
                [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"songdev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"veddev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"youini")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"downsou")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"downved")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"shazam")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"giff")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"stickkers")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"pictures")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"youtubb")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"soundcloud")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"insta")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"pentrist")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"likee")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"tiktok")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"facebook")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"tweter")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"snapchat")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"bookzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر تحميـل الكتب 📕](t.me/BDB0B) .
**- الامـر :**
**⪼** `.كتاب`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"savzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"storyzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر البحـث والتحميــل 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"telech")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر بحـث تيليـجـرام 🛰](t.me/BDB0B) .
**- الامـر :**
**⪼** `.تلي` **+ كلمـة**
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"telecg")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر بحـث تيليـجـرام 🛰](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"fundev")))
@check_owner
async def _(event):
    zelzal = "⤶ عـذراً عـزيـزي 🤷🏻‍♀\n⤶ هـذه اللوحه لا تشتغل في الخاص\n⤶ لـ إظهـار لوحـة المسـاعـدة 👇\n\n⤶ ارســل (.مساعده) في اي مجمـوعـه"
    try:
        await event.edit(
            "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر التسليـه والتحشيش 🏂🎃](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر التسليـه والتحشيش :**\n\n",
            buttons=[
                [
                    Button.inline("اوامــر تسليـه متحركـه", data="fun1dev"),
                ],
                [
                    Button.inline("اوامــر تسليـه جديـدة", data="fun2dev"),
                ],
                [
                    Button.inline("اوامــر التحـشيش", data="fun3dev"),
                ],
                [
                    Button.inline("اوامــر الالعــاب", data="fun4dev"),
                ],
                [Button.inline("رجــوع", data="devHELP")],
            ],
        link_preview=False)
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"fun1dev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر التسليـه 🏂](t.me/BDB0B) .
**⪼** `.تسليه1`
**⪼** `.تسليه2`
**⪼** `.تسليه3`
**⪼** `.تسليه4`
...
لا تقم بتكرار هذه الاوامـر او استخدامهـا بكثـره حتى لا يحدث تعليق لحسابك""",
        buttons=[
            [Button.inline("رجوع", data="fundev")],
        ],
    link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"fun2dev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر التسليـه ⛹🏻‍♀](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"fun3dev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر التحشيش 🎃](t.me/BDB0B) .
**- الامـر :**
**⪼** `.رفع تاج`
**⪼** `.رفع بقلبي`
**⪼** `.رفع مرتي`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"fun4dev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الالـعــاب 🎮🎳](t.me/BDB0B) .
**- الامـر :**
**⪼** `.العاب الجماعي`
...
@blal.tgbot.on(CallbackQuery(data=re.compile("broadcastz")))
@check_owner
async def dev_help(event):
    zelzal = "⤶ عـذراً عـزيـزي 🤷🏻‍♀\n⤶ هـذه اللوحه لا تشتغل في الخاص\n⤶ لـ إظهـار لوحـة المسـاعـدة 👇\n\n⤶ ارســل (.مساعده) في اي مجمـوعـه"
    try:
        await event.edit(
            "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الإذاعــة العـامـة 🏟️](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اقسـام الاوامـر المتعلقـه بالإذاعـة العـامـة فـي الحسـاب :**\n\n",
            buttons=[
                [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"broadcastgp")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الإذاعــة العـامـة 🏟️](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"broadcastpm")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الإذاعــة العـامـة 🏟️](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"broadcastk")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الإذاعــة العـامـة 🏟️](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"broadcastall")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼?? - اوامــر الإذاعــة العـامـة 🏟️](t.me/BDB0B) .
**- الامـر :**
**⪼** `.للكل`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"acccount")))
@check_owner
async def dev_help(event):
    zelzal = "⤶ عـذراً عـزيـزي 🤷🏻‍♀\n⤶ هـذه اللوحه لا تشتغل في الخاص\n⤶ لـ إظهـار لوحـة المسـاعـدة 👇\n\n⤶ ارســل (.مساعده) في اي مجمـوعـه"
    try:
        await event.edit(
            "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات الاوامـر المتعلقـه بالحسـاب :**\n\n",
            buttons=[
                [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"nextacc")))
@check_owner
async def dev_help(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات الاوامـر المتعلقـه بالحسـاب :**\n\n",
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"next2acc")))
@check_owner
async def dev_help(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات الاوامـر المتعلقـه بالحسـاب :**\n\n",
        buttons=[
            [
                Button.inline("الازعاج", data="echodev"),
                Button.inline("الانتحال", data="enthaldev"),
            ],
            [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"profcmd")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"echodev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"enthaldev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"gozzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"banzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"delzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"biome")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"namme")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"picme")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"channelme")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"groubme")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"pmme")))
@check_owner
async def _(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر البحث والتحميـل من جميـع مواقـع الـ سوشـل ميديـا :**\n\n",
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"pmcmd")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر حمـايــة الخــاص 🛡](t.me/BDB0B) .
**⪼** `.الحمايه تفعيل`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"pmvar")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - فــارات حمـايــة الخــاص 🛡](t.me/BDB0B) .
**⪼** `.اضف فار عدد التحذيرات`
**لـ تغييـر عدد تحذيرات امـر حماية الخاص التي يقوم البوت باعطائها للشخص الذي يراسلك خاص قبل حظـره**
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"infome")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"whome")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
https://t.me/dev_Thon/292

**لـ معرفـة معلومـات أي حسـاب على انستـاجـرام :**
https://t.me/dev_Thon/293
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"logme")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"mutme")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"banme")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"whonam")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الحســاب 🚹](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"extras")))
@check_owner
async def dev_help(event):
    zelzal = "⤶ عـذراً عـزيـزي 🤷🏻‍♀\n⤶ هـذه اللوحه لا تشتغل في الخاص\n⤶ لـ إظهـار لوحـة المسـاعـدة 👇\n\n⤶ ارســل (.مساعده) في اي مجمـوعـه"
    try:
        await event.edit(
            "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المـرفقــات 🖥](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر مرفقـات السـورس :**\n\n",
            buttons=[
                [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"meddia")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المـرفقــات 🖥](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"story")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الستـوريـات 🎆🏖](t.me/BDB0B) .
**- الامـر :**
**⪼** `.حالات واتس`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"avatar")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الآفتـــارات والصــور 🎆🏖](t.me/BDB0B) .
**- الامـر :**
**⪼** `.بنات`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"stickerrs")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر المـرفقــات 🖥](t.me/BDB0B) .
...
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀??𝗿𝗯𝗼𝘁 - اوامـر السبـام والتكـرار](t.me/ZTho) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"tooldev")))
@check_owner
async def dev_help(event):
    zelzal = "⤶ عـذراً عـزيـزي 🤷🏻‍♀\n⤶ هـذه اللوحه لا تشتغل في الخاص\n⤶ لـ إظهـار لوحـة المسـاعـدة 👇\n\n⤶ ارســل (.مساعده) في اي مجمـوعـه"
    try:
        await event.edit(
            "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر ادوات السـورس :**\n\n",
            buttons=[
                [
                    Button.inline("الاشتـراك الاجبـاري", data="subsdev"),
                ],
                [
                    Button.inline("الصيـد & الـتثبيت", data="huntdev"),
                ],
                [
                    Button.inline("تجميـع النقـاط", data="pointdev"),
                ],
                [
                    Button.inline("النشـر التلقائـي", data="nashdev"),
                ],
                [
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"nexttools")))
@check_owner
async def dev_help(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر ادوات السـورس :**\n\n",
        buttons=[
            ...
            [
                Button.inline("الحاسبة", data="calcdev"),
            ...
            [Button.inline("رجوع", data="tooldev")],
        ],
    link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"thatia")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"scanner")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"calcdev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"taks")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"transzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"creatzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"leavzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"scinczzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"urltools")))
@check_owner
async def dev_help(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - ادوات الروابــط 💡](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر ادوات الروابــط :**\n\n",
        buttons=[
            [
                Button.inline("اختصـار الروابـط", data="shorturl"),
            ],
            [
                Button.inline("سكريـن", data="screendev"),
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"shorturl")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
**- مثـال :**
`.اختصار https://github.com/dev-Thon/ZelZal`
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"screendev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"viewzzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"dnszzz")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"hideurl")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الادوات 💡](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"subsdev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الاشتــراك الاجبــاري 🛗](t.me/BDB0B) .
**- الامـر :**
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"huntdev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر الصيـد & الـتثبيت ❇️](t.me/BDB0B) .
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"pointdev")))
@check_owner
async def dev_help(event):
    await event.edit(
        "[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر تجميـع النقـاط 💡](t.me/BDB0B) .\n\n**⎉╎اليك عـزيـزي شـࢪوحـات اوامـر تجميـع النقـاط :**\n\n",
        buttons=[
            [
                Button.inline("نقـاط العـاب وعــد", data="waddev"),
            ],
            [Button.inline("رجوع", data="tooldev")],
        ],
    link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"waddev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر تجميــع النقــاط 🛂](t.me/BDB0B) .
...
`.راتب وعد 50`""",
        buttons=[
            [Button.inline("رجوع", data="pointdev")],
        ],
    link_preview=False)

@blal.tgbot.on(CallbackQuery(data=re.compile(rb"nashdev")))
@check_owner
async def _(event):
    await event.edit(
        """[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭  𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اوامــر النشــر التلقــائي 🌐](t.me/BDB0B  ) .
