...
from ..helpers.utils import _devutils, _format
...
MatrixalSP_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - اوامـر السبـام والتكـرار](t.me/BDB0B) 𓆪\n\n"
    "`.كرر` + عـدد + كلمـه\n"
...
async def spam_function(event, sandy, dev, sleeptimem, sleeptimet, DelaySpam=False):
    # sourcery no-metrics
    counter = int(dev[0])
    if len(dev) == 2:
        spam_message = str(dev[1])
        ...
        for _ in range(counter):
            ...
            await _devutils.unsavegif(event, sandy)
...
async def spammer(event):
    ...
    await spam_function(event, sandy, dev, sleeptimem, sleeptimet)
...
async def spammer(event):
    ...
    await event.delete()
    addgvar("spamwork", True)
    await spam_function(event, reply, dev, sleeptimem, sleeptimet, DelaySpam=True)


@blal.dev_cmd(pattern="تعبير مكرر$")
async def react_spam(event):
    msg = await event.get_reply_message()
    if not msg:
        return await edit_delete(event, "**- بالـرد على الرسـالة اولاً ...**", 10)
    devevent = await edit_or_reply(event, "**- جـارِ بدء التفاعـلات انتظـر ...**")
...
@blal.dev_cmd(pattern="ايقاف التكرار ?(.*)")
...
@blal.dev_cmd(pattern="التكرار")
async def cmd(Matrixallll):
    await edit_or_reply(Matrixallll, MatrixalSP_cmd)
