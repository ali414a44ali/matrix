...
# <https://www.github.com/dev-Thon/ZelZal/blob/master/LICENSE/>.
...
# =========================================================== #
#                           الملـــفـ كتـــابـــة مـــن الصفــــر - t.me/Matrix                           #
# =========================================================== #
Warn = "hhh"
BiLaL_BEST_SOURCE = "[ᯓ 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة خـاص 🚹](t.me/Matrix) .\n\n**- جـارِ الاذاعـه خـاص لـ أعضـاء الكـروب 🛗\n- الرجـاء الانتظـار .. لحظـات ⏳**"
BiLaL_PRO_SOURCE = "[ᯓ 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/Matrix) .\n\n**- جـارِ الاذاعـه لـ قائمـة زاجـل 📜\n- الرجـاء الانتظـار .. لحظـات ⏳**"
ZELZAL_PRO_DEV = "[ᯓ 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/Matrix) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⎉╎قائمـة الاذاعـه فـارغـه ؟! ❌**\n**⎉╎قم باضافـة يوزرات عبـر الامر**\n`.اضفـ زاجل` **بالـرد ع عدة يوزرات تفـصل بينهم مسـافـات**"
# =========================================================== #
...
@blal.dev_cmd(pattern=f"للكل(?: |$)(.*)", groups_only=True)
async def malath(event):
    BiLaL = event.pattern_match.group(1)
    if BiLaL:
    ...
    zelzal = await event.edit(BiLaL_BEST_SOURCE, link_preview=False)
    total = 0
    success = 0
    async for usr in event.client.iter_participants(event.chat_id):
        ...
        if zilzal.text:
            ...
            try:
                ...
                return
    ZELZAL_BEST_DEV = f"[ᯓ 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة خـاص 🚹](t.me/Matrix) .\n\n**⎉╎تمت الاذاعـه لـ اعضـاء الكـروب .. بنجـاح  ✅**\n**⎉╎عـدد {success} عضـو**"
...
@blal.dev_cmd(pattern="ايقاف للكل", groups_only=True)
...
async def malath(event):
    ...
    if BiLaL:
        ...
        return
    zilzal = await event.get_reply_message()
    if gvarstatus("ZAGL_dev") is None:
        return await event.edit(ZELZAL_PRO_DEV, link_preview=False)
    zelzal = gvarstatus("ZAGL_dev")
    users = zelzal.split(" ")
    zzz = await event.edit(BiLaL_PRO_SOURCE, link_preview=False)
    total = 0
    success = 0
    user_entity = None
    for user in users:
        ...
        if zilzal.text:
            try:
                user_entity = await blal.get_entity(user)
                if user_entity.bot or user_entity.deleted:
                    continue
                await blal.send_message(user_entity.id, zilzal, link_preview=False)
                success += 1
            ...
            try:
                ...
                if user_entity.bot or user_entity.deleted:
                    continue
                await blal.send_file(user_entity.id, zilzal.media, caption=zilzal.text)
                success += 1
            except UserAdminInvalidError:
                pass
            except Exception as e:
                zzz.edit(f"خطـأ فـي إرسـال الرسـالة إلــى {user_entity.id}: {str(e)}")
    ZELZAL_BEST_DEV = f"[ᯓ 𝗧𝗲𝗽𝘁𝗵𝗼𝗻 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/Matrix) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⎉╎تمت الاذاعـه .. بنجـاح  ✅**\n**⎉╎عـدد {success} أشخـاص**"
