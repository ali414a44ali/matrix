...
MatrixalWF_cmd = (
    ...
    "**⎞𝟕⎝** `.الترحيبات`\n"
    "**•• لـ جـلـب تـرحـيـبـك**\n\n\n"
    "**✾╎اوامـر الـردود التلقـائـيـه خـاص 🗣 :** \n"
    "**⎞𝟏⎝** `.اضف تلقائي`\n"
    "**•• ⦇ الامـر + كلمـة الـرد ⦈ بالـرد ع جملـة الـرد او بالـرد ع ميديـا**\n"
    ...
    "**✾╎اوامـر ردود البصمـات والميديـا 🎙:**\n"
    "**⎞𝟏⎝** `.بصمه`\n"
    "**•• ⦇ الامـر + كلمـة الـرد ⦈ بالـرد ع بصمـه او ميديـا**\n"
    "**•• لـ اضـافة رد بصمـه عـام**\n"
    "**⎞𝟐⎝** `.حذف بصمه`\n"
...
@blal.dev_cmd(pattern="الردود")
async def cmd(Matrixallll):
    await edit_or_reply(Matrixallll, MatrixalWF_cmd)

@blal.dev_cmd(pattern="الترحيب")
async def cmd(Matrixallll):
    await edit_or_reply(Matrixallll, MatrixalWF_cmd)
...
async def _(event):
    ...
    if gvarstatus("TIME_STOP") is not None: #Code by T.me/zzzzl1l
        devstop = gvarstatus("TIME_STOP")
        now = datetime.datetime.now().time()
        if datetime.time(f"{devstop}", 0) <= now < datetime.time(6, 0):
            return
    if (
        cws
