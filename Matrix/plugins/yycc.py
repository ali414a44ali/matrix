...
from ..helpers.utils import _format, reply_id, _devutils
from . import BOTLOG, BOTLOG_CHATID, blal
...
async def download_video(event):
    ...
    devevent = await edit_or_reply(event, "**⎉╎جـارِ التحميل انتظر قليلا ▬▭ ...**")
    reply_to_id = await reply_id(event)
    for url in urls:
        ytdl_data = await ytdl_down(devevent, video_opts, url)
        if ytdl_down is None:
            return
        try:
            f = pathlib.Path("dev_ytv.mp4")
            print(f)
            catthumb = pathlib.Path("dev_ytv.jpg")
            if not os.path.exists(catthumb):
                catthumb = pathlib.Path("dev_ytv.webp")
            if not os.path.exists(catthumb):
                catthumb = None
            await devevent.edit(
            ...
            uploaded = await event.client.fast_upload_file(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(
                        d, t, devevent, c_time, "Upload :", file_name=ytdl_data["title"]
                    )
                ),
            )
            ul.close()
    ...
    for url in urls:
        ...
        for _path in glob.glob(os.path.join(Config.TEMP_DIR, str(startTime), "*")):
            if _path.lower().endswith((".jpg", ".png", ".webp")):
                ...
                _fpath = _path
        if not _fpath:
            return await edit_delete(devevent, "__Unable to upload file__")
        await devevent.edit(
            f"**╮ ❐ جـارِ التحضيـر للـرفع انتظـر ...𓅫╰**:\
            \n**{vid_data['title']}***"
        )
...
async def _(event): #Code by T.me/zzzzl1l
    ...
    devevent = await edit_or_reply(event, "**╮ جـارِ البحث ؏ـن المقطـٓع الصٓوتـي... 🎧♥️╰**")
    ydl_ops = {
    ...
    try:
        ...
        for i in range(len(dur_arr) - 1, -1, -1):
            ...
            secmul *= 60
        await devevent.edit("**╮ جـارِ الرفـع ▬▬ . . .🎧♥️╰**")
        await event.client.send_file(
            event.chat_id,
            audio_file,
        ...
        #LOGS.error(str(err))
        return await devevent.edit("**- عـذرًا .. الوسـائـط مغلقـه هنـا ؟!**")
    except Exception as e:
        if "Requested format is not available." in str(e): # تبعي
            return await devevent.edit("**• هنالك تحديث جديد لـ مكتبة يوتيوب 📡**\n**• أرسِـل الامـر** ( `.تحديث البوت` )\n**• ثم انتظر 5 دقائق لـ إعادة تشغيـل البوت ⏳**\n**• بعدها تستطيع استخدام اوامر التحميل .. بدون مشاكـل ☑️**")
        else:
            return await devevent.edit(f"**• فشـل التحميـل** \n**• الخطـأ :** `{str(e)}`\n\n**• قم بـ إرســال هذا الخلل لـ مطور السورس لـ اصلاحه**\n**• تواصـل مطـور السـورس @o_p_g**")
    try:
...
async def _(event): #Code by T.me/zzzzl1l
    ...
    if event.pattern_match.group(1):
        ...
        return await edit_or_reply(event, "**⎉╎قم باضافـة إسـم للامـر ..**\n**⎉╎فيديو + اسـم الفيديـو**")
    devevent = await edit_or_reply(event, "**╮ جـارِ البحث ؏ـن الفيديـو... 🎧♥️╰**")
    ydl_opts = {
        "format": "bestvideo+bestaudio/best",  # Download best video and audio and merge
        "keepvideo": True,
        "prefer_ffmpeg": False,
    ...
    try:
        ...
        if "Requested format is not available." in str(e): # تبعي
            ...
            await devevent.edit("**• هنالك تحديث جديد لـ مكتبة يوتيوب 📡**\n**• أرسِـل الامـر** ( `.تحديث البوت` )\n**• ثم انتظر 5 دقائق لـ إعادة تشغيـل البوت ⏳**\n**• بعدها تستطيع استخدام اوامر التحميل .. بدون مشاكـل ☑️**")
    try:
        msg = await devevent.edit("**╮ جـارِ التحميل ▬▭ . . .🎧♥️╰**")
...
async def shazamcmd(event):
    ...
    devevent = await edit_or_reply(event, "**- جـار تحميـل المقـطع الصـوتي ...**")
    name = "dev.mp3"
    try:
    ...
    if flag == "s":
        ...
        async with event.client.conversation(chat) as conv:
            try:
                ...
                await blal(unblock("DeezerMusicBot"))
                purgeflag = await conv.send_message("/start")
...
async def download_video(event):
    ...
    for url in urls:
        ...
        try:
            f = pathlib.Path("dev_ytv.mp4")
            print(f)
            devthumb = pathlib.Path("dev_ytv.jpg")
            if not os.path.exists(devthumb):
                devthumb = pathlib.Path("dev_ytv.webp")
            if not os.path.exists(devthumb):
                devthumb = None
            await devevent.edit(
                f"**╮ ❐ جـارِ التحضيـر للـرفع انتظـر ...𓅫╰**:\
...
ZelzalMeMe_cmd = (
    ...
    "\n 𓆩 [𝙎𝙊𝙐𝙍𝘾𝞝 𝗧𝗘𝗣𝗧𝗛𝗢𝗡 ⌁](t.me/Matrix) 𓆪"
)


# Copyright (C) 2022 dev-Thon . All Rights Reserved
@blal.dev_cmd(pattern="البصمات")
...
@blal.dev_cmd(pattern="اضف تلقائي (.*)")
async def add_new_meme(event):
    keyword = event.pattern_match.group(1)
    string = event.text.partition(keyword)[2]
    msg = await event.get_reply_message()
    ...
    if add_pmrad(str(blal.uid), keyword, string, msg_id) is True:
        return await edit_or_reply(event, success.format("اضافة", keyword))
    remove_pmrad(str(blal.uid), keyword)
    if add_pmrad(str(blal.uid), keyword, string, msg_id) is True:
        return await edit_or_reply(event, success.format("تحديث", keyword))
    await edit_or_reply(event, "**⪼ ارسـل (** `.اضف تلقائي` **) + كلمـة الـرد**\n**⪼بالـرد ع جملـة او ميديـا 🗣**\n**⪼ لإضافتهـا لـ قائمـة ردودك التلقائيـة 🧾**")


@blal.dev_cmd(pattern="ردود الخاص$")
async def on_meme_list(event):
...
@blal.dev_cmd(pattern="حذف تلقائي(?: |$)(.*)")
async def remove_a_meme(event):
    filt = event.pattern_match.group(1)
    if not remove_pmrad(blal.uid, filt):
        await event.edit("**- ❝ الـرد التلقـائـي ↫** {} **غيـر موجـود ⁉️**".format(filt))
    else:
        await event.edit("**- ❝ الـرد التلقـائـي ↫** {} **تم حذفه .. بنجاح ☑️**".format(filt))


@blal.dev_cmd(pattern="حذف ردود الخاص$")
async def on_all_meme_delete(event):
    filters = get_pmrads(blal.uid)
    if filters:
        remove_all_pmrads(blal.uid)
