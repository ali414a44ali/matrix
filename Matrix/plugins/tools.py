...
async def scan(event):
    ...
    chat = "@VS_Robot"
    devevent = await edit_or_reply(event, " `Sliding my tip, of fingers over it`")
    async with event.client.conversation(chat) as conv:
        try:
            flag = await conv.send_message("/start")
        except YouBlockedUserError:
            await edit_or_reply(
                devevent, "**Error:** Trying to unblock & retry, wait a sec..."
            )
            await blal(unblock("VS_Robot"))
            flag = await conv.send_message("/start")
        ...
        if response1.text:
            ...
            await edit_delete(devevent, f"**Please wait for {sec}s before retry**", 15)
        else:
            await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            ...
            await event.client.send_read_acknowledge(conv.chat_id)
            if not input_str:
                await edit_or_reply(devevent, response3.text[30:])
            else:
...
async def parseqr(event):
    ...
    devevent = await edit_or_reply(event, "`Decoding....`")
    ...
    t_response, e_response = (await _devutils.runcmd(command_to_exec))[:2]
    if os.path.exists(downloaded_file_name):
        os.remove(downloaded_file_name)
    soup = BeautifulSoup(t_response, "html.parser")
    try:
        qr_contents = soup.find_all("pre")[0].text
        await edit_or_reply(devevent, f"**The decoded message is :**\n`{qr_contents}`")
    except IndexError:
        result = soup.text
        await edit_or_reply(devevent, f"**Failed to Decode:**\n`{result}`")
...
async def _(event):
    ...
    try:
        ...
        return await devevent.edit(str(e))
    end = datetime.now()
    ms = (end - start).seconds
    await edit_delete(devevent, "Created BarCode in {} seconds".format(ms))
...
async def _(event):
    ...
    devevent = await edit_or_reply(event, "`processiong...........`")
    input_str = event.pattern_match.group(1)
    xkcd_id = None
    if input_str:
    ...
    if not r.ok:
        return await devevent.edit("xkcd n.{} not found!".format(xkcd_id))
    data = r.json()
    year = data.get("year")
...
Year: {}""".format(
    ...
    await devevent.edit(output_str, link_preview=True)
