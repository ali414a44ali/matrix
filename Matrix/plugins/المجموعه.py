...
MatrixAL_RIGHTS = ChatBannedRights(until_date=None, view_messages=True, send_messages=True, send_media=True, send_stickers=True, send_gifs=True, send_games=True, send_inline=True, embed_links=True)
...
CHANNELS_STR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ **- 🝢 - احصـائيـات جميـع القنـوات** 𓆪\n\n"
CHANNELS_ADMINSTR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ **- 🝢 - احصـائيـات جميـع القنـوات اشـراف** 𓆪\n\n"
CHANNELS_OWNERSTR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ **- 🝢 - احصـائيـات جميـع القنـوات ملكيـة** 𓆪\n\n"
GROUPS_STR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ **- 🝢 - احصـائيـات جميـع المجمـوعـات** 𓆪\n\n"
GROUPS_ADMINSTR = "𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ **- 🝢 - احصـائيـات جميـع المجمـوعـات اشـراف** 𓆪\n\n"
...
async def ban_user(chat_id, i, rights):
    try:
        await blal(functions.channels.EditBannedRequest(chat_id, i, rights))
        return True, None
...
async def stats(event):  # sourcery no-metrics
    ...
    response = f"𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ **- 🝢 - احصـائيـات {full_name}** 𓆪\n"
...
async def stats(event):  # sourcery no-metrics
    ...
    devevent = await edit_or_reply(event, STAT_INDICATION)
    start_time = time.time()
    hi = []
    hica = []
    hico = []
...
#     chat = "@tgscanrobot"
#     devevent = await edit_or_reply(event, "`Processing...`")
#     async with event.client.conversation(chat) as conv:
#         try:
#             await conv.send_message(f"{uid}")
#         except Exception:
#             await edit_delete(devevent, "`unblock `@tgscanrobot` and then try`")
#         response = await conv.get_response()
#         await event.client.send_read_acknowledge(conv.chat_id)
#         await devevent.edit(re


@blal.dev_cmd(pattern="تجميع الاعضاء$")
...
@blal.dev_cmd(pattern="اضف الاعضاء$")
...
async def _(event):
    ...
    mentions = "𓆩 𝑺𝑶𝑼𝑹𝑪𝑬 𝙈𝙖𝙏𝙍𝙞𝙭  𝑮𝑹𝑶𝑼𝑷 𝑫𝑨𝑻𝑨 𓆪\n** ⪼ المشرفـون في ۿذه المجموعه :** \n"
    reply_message = await reply_id(event)
    input_str = event.pattern_match.group(1)
    to_write_chat = await event.get_input_chat()
    chat = None
    if input_str:
        mentions = f"𓆩 𝑺𝑶𝑼𝑹𝑪𝑬 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝑮𝑹𝑶𝑼𝑷 𝑫𝑨𝑻𝑨 𓆪\n** ⪼ المشرفـون في {input_str} :** \n"
        try:
...
async def get_users(show):
    ...
    devevent = await edit_or_reply(show, "**⎉╎جـارِ سحـب قائمـة معرّفـات الأعضـاء 🝛**")
    try:
        ...
        mentions += f" {str(e)}" + "\n"
    await edit_or_reply(devevent, mentions)
...
async def info(event):
    ...
    devevent = await edit_or_reply(event, "**⎉╎جـارِ جلـب معلومـات الدردشـة، إنتظـر ⅏**")
    ...
    try:
        ...
        await devevent.edit("**⎉╎حـدث خـطأ مـا، يرجـى التحقق من الأمـر ⎌**")
...
async def get_chatinfo(event, devevent):
    ...
    try:
        ...
        try:
            ...
            await devevent.edit("**⎉╎لـم يتـمّ العثـور على القنـاة/المجموعـة ✕**")
            return None
            ...
            await devevent.edit("**⎉╎القنـاة أو المجموعـة الخارقـة غيـر موجـودة ✕**")
            return None
        except (TypeError, ValueError) as err:
            LOGS.info(err)
            await edit_delete(devevent, "**⎉╎لم يتم العثور على المجموعة او القناة**")
            return None
    return chat_info
...
async def _(event):  # sourcery no-metrics
    ...
    async for i in event.client.iter_participants(event.chat_id):
        ...
        if i.bot:
            ...
            n += 1
    if input_str:
        required_string = """𓆰 𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ **- 🝢 - معـلومـات المجمـوعــه** 𓆪\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻 
...
𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻"""
        ...
        await sleep(5)
    await et.edit(
        """𓆰 𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ **- 🝢 - معـلومـات المجمـوعــه** 𓆪\n 𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻
⪼ **العدد ↫ {} **مستخدماً
⪼ **الحسابات المحذوفه ↫** {}
⪼ **اخر ظهور منذ زمن طويل ↫** {}
⪼ **اخر ظهور منذ شهر ↫** {}
...
@blal.dev_cmd(pattern="رفع الحظر ?(.*)")
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
...
@blal.dev_cmd(pattern=r"مسح المحظورين(.*)")
async def _(event):
    devevent = await edit_or_reply(event, "**⎉╎ إلغاء حظر جميع الحسابات المحظورة في هذه المجموعة 🆘**")
    succ = 0
    total = 0
    flag = False
    chat = await event.get_chat()
    async for i in event.client.iter_participants(event.chat_id, filter=ChannelParticipantsKicked, aggressive=True):
        ...
        try:
            ...
            await devevent.edit(f"**⎉╎{readable_time(e.seconds)} مطلـوب المـعاودة مـرة اخـرى للـمسح 🔁 **")
            await sleep(e.seconds + 5)
        except Exception as ex:
            await devevent.edit(str(ex))
        else:
            succ += 1
            if flag:
                await sleep(2)
            ...
            try:
                if succ % 10 == 0:
                    await devevent.edit(f"**⎉╎جـارِ مسـح المحـظورين ⭕️  : \n {succ} الحسـابات الـتي غيـر محظـورة لحـد الان.**")
            except MessageNotModifiedError:
                pass
    await devevent.edit(f"**⎉╎تـم مسـح المحـظورين مـن أصـل 🆘 :**{succ}/{total} \n اسـم المجـموعـة 📄 : {chat.title}")

@blal.dev_cmd(pattern=r"المحذوفين ?([\\s\\S]*)")
...
@blal.dev_cmd(pattern="احصائياتي$")
async def count(event):
    ...
    result += f"𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ **- 🝢 - احصـائيـات الحسـاب** 𓆪\n"
...
@blal.dev_cmd(pattern="الرابط ?(.*)")
async def dev(event):
    devevent = await edit_or_reply(event, "**⇜ جـارِ جلـب رابـط المجموعـه ⇜**")
    chat = await event.get_chat()
    try:
        ...
        return await edit_delete(devevent, "**⎉╎عـذراً عـزيـزي .. انت لسـت مشرفـاً هنـا 🙇🏻**", 5)
    await devevent.edit(f"**⎉╎رابـط الـمجموعـه ⎋:**\n\n⎌ [{chat.title}]({ZL.link}) ⎌")   


@blal.dev_cmd(pattern="رسائلي$")
async def dev(event):
    zzm = "me"
...
@blal.dev_cmd(pattern="(الرسائل|رسائل) ?(.*)")
async def dev(event):
    k = await event.get_reply_message()
    if k:
        a = await bot.get_messages(event.chat_id, 0, from_user=k.sender_id)
...
@blal.dev_cmd(pattern="الحاظرهم$")
async def main(event):
    result = await blal(functions.contacts.GetBlockedRequest(offset=0, limit=1000000))
    alist = []
    for user in result.users:
        if not user.bot:
            ...
            alist.append(zzz)
    if alist:
        await blal.send_message("me", "\n".join(alist))


@blal.dev_cmd(pattern="قيد (.*)")
async def _(event):
    exe = event.text[5:]
    try:
        result = await blal(
...
@blal.dev_cmd(pattern="احذف (.*)")
async def _(event):
    exe = event.text[5:]
    await blal.get_dialogs()
    chat = exe
    await blal.delete_dialog(chat, revoke=True)
