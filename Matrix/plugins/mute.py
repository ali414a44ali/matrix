...
async def startgmute(event):
    if event.is_private:
        ...
        if user.id == 7645521594 or user.id == 7645521594:
    ...
    if BOTLOG:
        ...
        if reason:
            if add_katm(str(blal.uid), str(user.id), user.first_name, reason) is True:
...
@blal.dev_cmd(pattern="الغاء كتم(?: |$)(.*)")
async def endgmute(event):
    ...
    if BOTLOG:
        if not remove_katm(str(blal.uid), str(user.id)):
            if reason:
                await event.client.send_message(
                    BOTLOG_CHATID,
...
@blal.dev_cmd(incoming=True, forword=True)
...
@blal.dev_cmd(pattern="مسح المكتومين$")
async def on_all_muted_delete(event):
    mktomers = get_katms(blal.uid)
    count = 1
    if mktomers:
        dev = await edit_or_reply(event, "**⪼ جـارِ مسـح المكتوميـن .. انتظـر ⏳**")
        for mktoom in mktomers:
            unmute(mktoom.ktm_id, "gmute")
            count += 1
        remove_all_katms(blal.uid)
        await dev.edit("**⪼ تم حـذف جميـع المكتوميـن .. بنجـاح ✅**")
    else:
        OUT_STR = "**- لايــوجـد لديــك أي مكتوميــن بعــد 🔔**"
...
@blal.dev_cmd(pattern="ستوري جهاتي(?: |$)(.*)")
async def story_upload(event):
    reply = await event.get_reply_message()
    r_caption = reply.text if reply.text else ""
    command_caption = event.pattern_match.group(1)
    dev = await edit_or_reply(event, "**⎉╎جـارِ رفـع الستوري (القصه) ع حسابك ...** ")
    if not reply:
        return await dev.edit("**⎉╎بالـرد ع صـورة او فيـديـو او اكثـر**\n**⎉╎لـ رفعهـا ستـوري ع حسابك ...**")
    if not reply.media:
        return await dev.edit("**⎉╎بالـرد ع صـورة او فيـديـو او اكثـر**\n**⎉╎لـ رفعهـا ستـوري ع حسابك ...**")
    ...
    if hasattr(reply, 'media_group_id'):
        if reply.video or (reply.document and reply.document.mime_type.startswith('video')):  # التحقق من الفيديو
            async for message in blal.iter_messages(reply.chat_id, filter=types.InputMessagesFilterDocument, grouped=reply.grouped_id):
            ...
            async for message in blal.iter_messages(reply.chat_id, filter=types.InputMessagesFilterPhotos, grouped=reply.grouped_id):
                if message.grouped_id == reply.grouped_id and message.photo:
                    media_files.append(message.media)
        else: #  في حالة كان الملف ليس صورة او فيديو
            return await dev.edit("⎉╎نوع الوسائط غير  مدعوم, الرجاء اختيار صورة او فيديو ❌")
    if not media_files:
        return await dev.edit("**⎉╎لم يتم العثور على وسائط ❌**\n**⎉╎حاول مرة اخرى في وقت لاحق ...⏳**")
    for media in media_files:
        try:
            file_path = await blal.download_media(media, file="./downloaded_media/")
            await dev.edit(f"**⎉╎تم تحميل الميديا إلى** `{file_path}` ☑️\n**⎉╎جـارِ الرفـع الآن .. انتظر قليلاً ▬▭**")
            try:
                file = await blal.upload_file(file_path)
                ...
                try:
                    if media.video and reply.video:
                        ...
                        await dev.edit("**- وسائط غيـر مدعومـه ❌**")
                        continue
                except Exception as e:
                    if media.photo and reply.photo:
                        ...
                        await dev.edit("**- وسائط غيـر مدعومـه ❌**")
                        continue
                result = await blal(functions.stories.SendStoryRequest(
                    media=input_media,
                ...
                if hasattr(result, 'updates') and result.updates:
                    if hasattr(result.updates[0], 'id'):
                        ...
                        zuz = blal.me.username if blal.me.username else ""
                        story_link = f"https://t.me/{zuz}/s/{story_id}"
                        await dev.edit(f"**⎉╎تم رفـع القصة لحسابك .. بنجـاح ☑️**\n**⎉╎مسار الملف:** `{file_path}`\n**⎉╎رابـط الستوري: {story_link}**")
                    else:
                        await dev.edit(f"**⎉╎تم رفـع القصة لحسابك .. بنجـاح ☑️**\n**⎉╎مسار الملف:** `{file_path}`")
                else:
                    await dev.edit(f"**⎉╎تم رفـع القصة لحسابك .. بنجـاح ☑️**\n**⎉╎مسار الملف:** `{file_path}`")
                ...
                if "STORIES_TOO_MUCH" in str(e):
                    return await dev.edit(f"**- اووبـس .. لقد تجاوزت الحد الأقصى ❌**\n**- لعدد القصص المسموح بها على تيليجرام 🚸**")
                elif "A premium account is required to execute this action" in str(e):
                    return await dev.edit(f"**- حسابك لا يدعم رفع القصص ❌**\n**- قم بالترقيه الى البريميوم 🌟**")
                else:
                    return await dev.edit(f"**- اووبـس .. حدث خطأ اثناء رفع القصة ❌**\n**- الخطـأ:**\n{str(e)}")
        except Exception as e:
            return await dev.edit(f"**- اووبـس .. حدث خطأ اثناء رفع القصة ❌**\n**- الخطـأ:**\n{str(e)}")


@blal.dev_cmd(pattern="ستوري عام(?: |$)(.*)")
async def story_upload(event):
    reply = await event.get_reply_message()
    r_caption = reply.text if reply.text else ""
    command_caption = event.pattern_match.group(1)
...
async def download_all_stories(event, user_id):
    stories = await blal(functions.stories.GetPeerStoriesRequest(
        peer=user_id
    ))
    
    for story in stories.stories.stories:
        file = await blal.download_media(story.media)
        caption = story.caption if story.caption else "" # Get caption if exists
        yield file, caption 

@blal.dev_cmd(pattern="ستوريات(?: |$)(.*)")
async def story_upload(event):
    ...
    if not input_str and reply_message:
        ...
        username = user.id
    dev = await edit_or_reply(event, f"**⎉╎جـارِ تحميـل ستوريات المستخدم {username}**\n**⎉╎الرجاء الانتظار لحظات ...⏳**")
    try:
        user = await event.client.get_entity(username)
    except Exception:
        return await dev.edit("**- عــذࢪاً .. لايمكــنني العثــوࢪ علـى المسـتخــدم ؟!**")
    user_id = user.id
    try:
        async for story, caption in download_all_stories(event, user_id):
            ...
            await blal.send_file(event.chat_id, story, caption=captioon, parse_mode="html")
    except Exception as e:
        await dev.edit(f"- اووبـس .. حدث خطأ اثناء تحميل القصص ❌\n- الخطـأ:\n{str(e)}")
    await dev.delete()
