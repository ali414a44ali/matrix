...
from ..vc_Matrix.vcp_helper import devVC
...
async def joinVoicechat(event):
    ...
    try:
        vc_chat = await blal.get_entity(chat)
...
ZelzalMusic_cmd = (
"[ᯓ تيبثـون - أوامـر الصوتيـات 😀](t.me/Matrix) ."
"**⋆─┄─┄─┄─┄──┄─┄─┄─┄─⋆**\n"
"⚉ `.شغل`\n"
...
@blal.dev_cmd(pattern="الميوزك")
async def cmd(zelzallll):
    await edit_or_reply(zelzallll, ZelzalMusic_cmd)

@blal.dev_cmd(pattern="ميوزك")
async def cmd(zelzallll):
    await edit_or_reply(zelzallll, ZelzalMusic_cmd)
