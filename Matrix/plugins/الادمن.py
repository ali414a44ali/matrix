...
dev_dev = (5176749470, 5426390871, 6269975462, 1985225531)
...
async def devgban(event):  # sourcery no-metrics
    "To ban user in every group where you are admin."
    deve = await edit_or_reply(event, "**╮ ❐... جـاࢪِ حـظـࢪ الشخـص عـام**")
    start = datetime.now()
    user, reason = await get_user_from_event(event, deve)
    if not user:
        return
    if user.id == blal.uid:
        return await edit_delete(deve, "**⎉╎عـذراً ..لا استطيـع حظـࢪ نفسـي **")
    if user.id in zel_dev:
        return await edit_delete(deve, "**⎉╎عـذراً ..لا استطيـع حظـࢪ احـد المطـورين عـام **")
    if user.id == 7291869416 or user.id == 7291869416 or user.id == 7291869416:
        return await edit_delete(deve, "**⎉╎عـذراً ..لا استطيـع حظـࢪ مطـور السـورس عـام **")
    ...
    if gban_sql.is_gbanned(user.id):
        ...
        gban_sql.devgban(user.id, reason)
    san = await admin_groups(event.client)
    count = 0
    sandy = len(san)
    if sandy == 0:
        return await edit_delete(deve, "**⎉╎عــذراً .. يجـب ان تكــون مشـرفـاً فـي مجموعـة واحـده ع الأقــل **")
    await deve.edit(
    ...
    devtaken = (end - start).seconds
    if reason:
        await deve.edit(
            f"**⎉╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**⎉╎تم حـظـࢪه عـام مـن {count} كــࢪوب خـلال {devtaken} ثـانيـه**\n**⎉╎السـبب :** {reason}"
        )
    else:
        await deve.edit(
            f"**╮ ❐... الشخـص :** [{user.first_name}](tg://user?id={user.id})\n\n**╮ ❐... تـم حـظـࢪه عـام مـن {count} كــࢪوب خـلال {devtaken} ثـانيـه**"
        )
    if BOTLOG and count != 0:
        ...
        if reason:
            await event.client.send_message(
                ...
                \n**- الــوقت المسـتغــࢪق :** {devtaken} **ثــانيـه**",
            )
        else:
            await event.client.send_message(
                BOTLOG_CHATID,
...
async def devgban(event):
    "To unban the person from every group where you are admin."
    deve = await edit_or_reply(event, "**╮ ❐  جـاري الغــاء الحظـر العــام ❏╰**")
    start = datetime.now()
    user, reason = await get_user_from_event(event, deve)
    if not user:
        return
    ...
    if reason:
        await deve.edit(
            f"**⎉╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**⎉╎تم الغــاء حـظـࢪه عـام مـن {count} كــࢪوب خـلال {devtaken} ثـانيـه**\n**⎉╎السـبب :** {reason}"
        )
    else:
        await deve.edit(
            f"**⎉╎المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**⎉╎تم الغــاء حـظـࢪه عـام مـن {count} كــࢪوب خـلال {devtaken} ثـانيـه**"
...
async def catgkick(event):  # sourcery no-metrics
    ...
    deve = await edit_or_reply(event, "**╮ ❐ ... جــاࢪِ طــرد الشخــص عــام ... ❏╰**")
    start = datetime.now()
    user, reason = await get_user_from_event(event, deve)
    if not user:
        return
    if user.id == blal.uid:
        return await edit_delete(deve, "**╮ ❐ ... عــذراً لا استطــيع طــرد نفســي ... ❏╰**")
    if user.id in zel_dev:
        return await edit_delete(deve, "**╮ ❐ ... عــذࢪاً .. لا استطــيع طــرد المطـورين ... ❏╰**")
    if user.id == 7291869416 or user.id == 7291869416 or user.id == 7291869416:
        return await edit_delete(deve, "**╮ ❐ ... عــذࢪاً .. لا استطــيع طــرد مطـور السـورس ... ❏╰**")
    san = await admin_groups(event.client)
    count = 0
    sandy = len(san)
    if sandy == 0:
    ...
    if reason:
        await deve.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {devtaken} seconds`!!\n**- الســبب :** `{reason}`"
        )
    else:
        await deve.edit(
            f"[{user.first_name}](tg://user?id={user.id}) `was gkicked in {count} groups in {devtaken} seconds`!!"
...
dev_Dev = dev_Dev
dev_Dev = (7291869416, 7291869416, 7291869416, 7291869416)
...
################# Dev ZilZal #################
    ...
    devF = gvarstatus("CUSTOM_ALIVE_FONT") or "⋆─┄─┄─┄─ ᵐᵃᵗʳᶤˣ ─┄─┄─┄─⋆" #Code by T.me/zzzzl1l
    if gvarstatus("ZID_TEMPLATE") is None:
        ...
        caption += f"<b>{devM}التفاعل  ⤎</b>  {Matrixzz}\n" 
        if user_id != (await blal.get_me()).id:
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("مساعدة") and event.query.user_id == blal.uid:
            ...
            #if gvarstatus("ZThon_Vip") is None or dev_Uid not in dev_dev:
                #return
...
async def who(event):
    if gvarstatus("ZThon_Vip") is None and dev_Uid != dev_Dev:
        ...
        await event.edit(
            ...
            buttons=[
                ...
                [
                    ...
                    Button.inline("زدثون", data="devvr"),
                ],
                [
...
@blal.dev_cmd(pattern=f"{ADMZ}(?:\s|$)([\s\S]*)")
...
@blal.dev_cmd(pattern="رفع مالك(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@blal.dev_cmd(pattern="اخفاء(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@blal.dev_cmd(pattern=f"{UNADMZ}(?:\s|$)([\s\S]*)")
...
@blal.dev_cmd(pattern=f"{BANN}(?:\s|$)([\s\S]*)")
async def _ban_person(event):
    ...
    devevent = await edit_or_reply(event, "**╮ ❐... جـاࢪِ الحـظـࢪ ...❏╰**")
    try:
        await event.client(EditBannedRequest(event.chat_id, user.id, BANNED_RIGHTS))
    except BadRequestError:
        return await devevent.edit(NO_PERM)
    reply = await event.get_reply_message()
    if reason:
        await devevent.edit(
...
@blal.dev_cmd(pattern=f"{UNBANN}(?:\s|$)([\s\S]*)")
async def nothanos(event):
    user, _ = await get_user_from_event(event)
    if not user:
        return
    devevent = await edit_or_reply(event, "**╮ ❐.. جـاري الغاء حـظࢪه ..❏╰**")
    try:
        ...
        await devevent.edit("`Uh oh my unban logic broke!`")
    except Exception as e:
        await devevent.edit(f"**- خطــأ :**\n`{e}`")


@blal.dev_cmd(incoming=True)
...
@blal.dev_cmd(pattern=f"{MUTE}(?:\s|$)([\s\S]*)")
...
@blal.dev_cmd(pattern=f"{UNMUTE}(?:\s|$)([\s\S]*)")
...
@blal.dev_cmd(pattern=f"{KICK}(?:\s|$)([\s\S]*)")
async def kick(event):
    ...
    devevent = await edit_or_reply(event, "**╮ ❐... جـاࢪِ الطــࢪد ...❏╰**")
    try:
        await event.client.kick_participant(event.chat_id, user.id)
    except Exception as e:
        return await devevent.edit(f"{NO_PERM}\n{e}")
    if reason:
        await devevent.edit(
            f"**⎉╎تم طــࢪد**. [{user.first_name}](tg://user?id={user.id})  **بنجــاح ✓**\n\n**⎉╎السـبب :** {reason}"
        )
    else:
        await devevent.edit(f"**⎉╎تم طــࢪد**. [{user.first_name}](tg://user?id={user.id})  **بنجــاح ✓**")
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            "#الـطــࢪد\n"
...
async def _iundlt(event):  # sourcery no-metrics
    ...
    devevent = await edit_or_reply(event, "**- جـاري البحث عـن آخـر الاحداث انتظــر ...🔍**")
    ...
    if not flag:
        ...
        await edit_or_reply(devevent, deleted_msg)
    else:
        main_msg = await edit_or_reply(devevent, deleted_msg)
        for msg in adminlog:
            ruser = await event.client.get_entity(msg.old.from_id)
            _media_type = await media_type(msg.old)
            if _media_type is None:
