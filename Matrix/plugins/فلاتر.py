...
from ..helpers import asciiart, dev_meeme, dev_meme, media_type, meme_type
...
async def memes(event):
    ...
    devinput = event.pattern_match.group(2)
    reply = await event.get_reply_message()
    ...
    if gvarstatus("dev_FONTS") is None:
        dev_FONTS = "Matrix/helpers/styles/Terserah.ttf"
    else:
        dev_FONTS = gvarstatus("dev_FONTS")
    if max(len(top), len(bottom)) < 21:
        await dev_meme(dev_FONTS, top, bottom, meme_file, meme)
    else:
        await dev_meeme(top, bottom, dev_FONTS, meme_file, meme)
...
async def memes(event):
    ...
    devinput = event.pattern_match.group(1)
    devinput = int(devinput) if devinput else 50
    reply = await event.get_reply_message()
    if not reply:
        return await edit_delete(event, "**⎉╎قم بالـرد ع وسـائط مدعومـه ...**")
    catid = await reply_id(event)
    ...
    try:
        await crop(meme_file, outputfile, devinput)
    except Exception as e:
...
async def memes(event):
    ...
    if not devinput:
        devinput = "50"
    if ";" in str(devinput):
        devinput, colr = devinput.split(";", 1)
    else:
        colr = 0
    ...
    try:
        await add_frame(meme_file, outputfile, devinput, colr)
    except Exception as e:
        return await output[0].edit(f"`{e}`")
    try:
        await event.client.send_file(
...
@blal.dev_cmd(pattern="صور (.*)")
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
    await event.edit("**╮ ❐ جـاري البحث عن الصـور  ...𓅫╰**")
    devthon = event.pattern_match.group(1)
    ...
    if not url_lst:
        await event.edit(f"**- اووبـس .. لم استطـع ايجـاد صـور عـن {devthon} ؟!**\n**- حـاول مجـدداً واكتـب الكلمـه بشكـل صحيح**")
        return
    await event.reply(
        file=url_lst,
        parse_mode="html",
    ...
    await event.edit(
        f"**- اكتمـل البحث عـن {devthon} في {ms} ثانيـه ✓**",
        link_preview=False
    )
...
async def _(event):
    ...
    async with aiohttp.ClientSession() as requests:
        ...
        for result in response["results"]:
            ...
            url_lst.append(image_save_path)
    if not url_lst:
        await event.edit(f"**- اووبـس .. لم استطـع ايجـاد خلفيـات عـن {devthon} ؟!**\n**- حـاول مجـدداً واكتـب الكلمـه بشكـل صحيح**")
        return
    await event.reply(
        file=url_lst,
        parse_mode="html",
