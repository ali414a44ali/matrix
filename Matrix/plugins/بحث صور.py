...
async def _(event):
    ...
    if not url_lst:
        await event.edit(f"**- اووبـس .. لم استطـع ايجـاد صـور عـن {BiLaL} ؟!**\n**- حـاول مجـددًا واكتـب الكلمـة بشكـل صحيح**")
        return
    await event.reply(
        file=url_lst,
        parse_mode="html",
    ...
    await event.edit(
        f"**- اكتمـل البحث عـن {BiLaL} في {ms} ثانيـة ✓**",
        link_preview=False
    )
...
async def _(event):
    ...
    async with aiohttp.ClientSession() as requests:
        ...
        for result in response["results"]:
            ...
            url_lst.append(image_save_path)
    if not url_lst:
        await event.edit(f"**- اووبـس .. لم استطـع ايجـاد خلفيـات عـن {BiLaL} ؟!**\n**- حـاول مجـددًا واكتـب الكلمـة بشكـل صحيح**")
        return
    await event.reply(
        file=url_lst,
        parse_mode="html",
