...
from . import edit_delete, blal, logging
...
autopic_path = os.path.join(os.getcwd(), "BDB0B", "original_pic.png")
digitalpic_path = os.path.join(os.getcwd(), "BDB0B", "digital_pic.png")
autophoto_path = os.path.join(os.getcwd(), "BDB0B", "photo_pfp.png")
    ...
    while DIGITALPICSTART:
        if not os.path.exists(digitalpic_path):
            ...
            while not downloader.isFinished():
                pass
        devfont = gvarstatus("DEFAULT_PIC") or "BDB0B/helpers/styles/Papernotes.ttf"
        shutil.copy(digitalpic_path, autophoto_path)
        Image.open(autophoto_path)
        current_time = datetime.now().strftime("%I:%M")
        img = Image.open(autophoto_path)
        drawn_text = ImageDraw.Draw(img)
        fnt = ImageFont.truetype(f"{devfont}", 35)
        drawn_text.text((140, 70), current_time, font=fnt, fill=(280, 280, 280))
        img.save(autophoto_path)
        file = await blal.upload_file(autophoto_path)
        try:
            if i > 0:
                await blal(
                    functions.photos.DeletePhotosRequest(
                        await blal.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await blal(functions.photos.UploadProfilePhotoRequest(file))
...
async def autoname_loop():
    while AUTONAMESTART := gvarstatus("autoname") == "true":
        ...
        devT = gvarstatus("CUSTOM_ALIVE_EMdev") or "⏐"
        name = f"{HM}{devT}"
        LOGS.info(name)
        try:
            await blal(functions.account.UpdateProfileRequest(first_name=name))
    ...
    while AUTOBIOSTART:
        ...
        for normal in HM:
            if normal in normzltext:
              ...
              namefont = namerzfont[normzltext.index(normal)]
              HM = HM.replace(normal, namefont)
        DEFAULTUSERBIO = gvarstatus("DEFAULT_BIO") or "الحمد الله على كل شئ - @BiLaL"
        bio = f"{DEFAULTUSERBIO} ⏐ {HM}"
        LOGS.info(bio)
        try:
            await blal(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(CHANGE_TIME)
...
blal.loop.create_task(digitalpicloop())
blal.loop.create_task(autoname_loop())
blal.loop.create_task(autobio_loop())
