...
async def zelzal_gpt(event):
    ...
    dev = await edit_or_reply(event, "**𓆰جـار إنشـاء ايميـل وهمـي 📧...**")
    async with borg.conversation(chat) as conv: # code by t.me/zzzzl1l
        try:
            ...
            BiLaL = await conv.get_response()
            malath = BiLaL.text
            if "📧 Your temporary email" in BiLaL.text:
...
async def zelzal_gpt(event):
    chat = "@TeMail_Robot" # code by t.me/zzzzl1l
    dev = await edit_or_reply(event, "**𓆰جـار جلب رسائـل البريـد 📬...**")
    async with borg.conversation(chat) as conv: # code by t.me/zzzzl1l
        try:
            await conv.send_message("/start")
            await conv.get_response()
