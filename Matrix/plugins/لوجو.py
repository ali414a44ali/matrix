...
async def bad(event):
    ...
    if input_str not in bg_name:
        devevent = await edit_or_reply(event, "`Give me a correct background name...`")
        await asyncio.sleep(1)
        await edit_delete(devevent, lbg_list, time=60)
    else:
...
async def pussy(event):  # sourcery no-metrics
    ...
    if cmd == "":
        ...
        if input_str not in logo_font:
            devevent = await edit_or_reply(event, "`Give me a correct font name...`")
            await asyncio.sleep(1)
            await edit_delete(devevent, font_name, time=80)
        else:
            if " " in input_str:
        ...
        if input_str not in fg_name:
            devevent = await edit_or_reply(event, "`Give me a correct color name...`")
            await asyncio.sleep(1)
            await edit_delete(
                devevent,
                f"**Available color names are here:-**\n\n{fg_list}",
