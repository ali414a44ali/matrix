...
async def _(event):  # sourcery no-metrics
    ...
    uid = user.id
    chat = "@SangMata_BOT"
    devevent = await edit_or_reply(event, "**⎉╎جـارِ الكشـف ...**")
    async with event.client.conversation(chat) as conv:
        try:
            await conv.send_message(f"{uid}")
        except YouBlockedUserError:
            await edit_delete(devevent, "**- اضغط ستارت هنـا @SangMata_BOT ثم اعد ارسال الامر**")
        responses = []
        while True:
            try:
    ...
    if not responses:
        await edit_delete(devevent, "**- الامـر في وضع الصيانه حاليـاً ...**")
    if "No data available" in responses:
        await edit_delete(devevent, "**⎉╎المستخدم ليس لديه أي سجل اسمـاء بعـد ...**")
    ...
    for i in check:
        if sandy:
            ...
            sandy = True
            await devevent.edit(i, parse_mode=_format.parse_pre)
