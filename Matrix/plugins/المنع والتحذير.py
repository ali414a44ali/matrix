...
from . import blal, BOTLOG_CHATID
...
@blal.dev_cmd(incoming=True, groups_only=True)
...
@blal.dev_cmd(pattern="تحذير(?:\\s|$)([\\s\\S]*)")
async def _(event):
...
@blal.dev_cmd(pattern="التحذيرات")
async def _(event):
    reply_message = await event.get_reply_message()
    if not reply_message:
...
@blal.dev_cmd(pattern="حذف التحذيرات(?: |$)(.*)")
async def _(event):
    reply_message = await event.get_reply_message()
    sql.reset_warns(reply_message.sender_id, event.chat_id)
