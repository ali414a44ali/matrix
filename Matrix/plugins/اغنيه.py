...
async def _(event):
    ...
    async with borg.conversation(chat) as conv: 
        try:
            ...
            Matrixal = await conv.get_response()
            if "⏳" not in Matrixal.text:
                await Matrixal.click(0)
                await asyncio.sleep(5)
                Matrixal = await conv.get_response()
                await event.delete()
...
async def _(event):
    ...
    async with borg.conversation(chat) as conv: 
        try:
            ...
            Matrixal = await conv.get_response()
            if "Version:" in Matrixal.text:
                await Matrixal.click(text='Download')
                await asyncio.sleep(5)
                Matrixal = await conv.get_response()
                zilzal = Matrixal.text
                if "above 50MB" in Matrixal.text:
...
@blal.dev_cmd(pattern="رابط(?:\\s|$)([\\s\\S]*)")
async def linkapk(event):
    ...
    dev = await edit_or_reply(event, "**⎉╎جـارِ البحث عن روابـط التطبيق ...**")
    async with event.client.conversation(chat) as conv:
        try:
            await conv.send_message(input_str)
        except YouBlockedUserError:
            await blal(unblock("apkdl_bot"))
            await conv.send_message(input_str)
        await asyncio.sleep(5)
