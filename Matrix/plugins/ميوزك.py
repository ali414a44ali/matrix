...
@blal.on(events.NewMessage(pattern=r"^\.يوت\s+(.*)"))
async def forward_to_group(event):
    ...
    # نرسل للقروب
    sent_msg = await blal.send_message(CHECK_GROUP_LINK, f"يوت {input_text}")

    replies = {"count": 0}

    @blal.on(events.NewMessage(chats=CHECK_GROUP_LINK))
    async def reply_handler(reply_event):
        ...
        if replies["count"] == 1:
            ...
            blal.remove_event_handler(reply_handler)
