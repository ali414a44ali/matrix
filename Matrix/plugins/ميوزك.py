...
new_blal = TelegramClient(StringSession(Config.STRING_SESSION), Config.APP_ID, Config.API_HASH)

async def PyStart():
    global blal_py
    try:
        await new_blal.start()
        blal_py = PyTgCalls(new_blal)
        await blal_py.start()
...
async def LeaveStream(chat_id):
    global blal_py
    await blal_py.leave_group_call(
        chat_id,
    )
...
@blal.dev_cmd(pattern="دعوه للمكالمه(?: |$)(.*)")
async def _(e):
...
@blal.dev_cmd(pattern="بدء مكالمه(?: |$)(.*)")
async def _(e):
    try:
...
@blal.on(events.NewMessage(outgoing=True, pattern=r'.شغل صوت'))
...
@blal.on(events.NewMessage(outgoing=True, pattern=r'.شغل فيديو'))
