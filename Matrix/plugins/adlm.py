...
@blal.dev_cmd(pattern="^\.تعلم (.+?) الجواب (.+)$")
...
async def copy_channel_messages(event):
    try:
        ...
        async for post in blal.iter_messages(channel_username, reverse=True):
...
async def _(Matrixal):
    ...
    async with _INVITING_LOCK:
                ...
                if (
                    ...
                    try:
                        if error.lower().startswith(("Too many", "a wait of")) or success > max_success:
                            ...
                            return await blal.send_message(BOTLOG_CHATID, done_limit, parse_mode="html")
                        await ga(
                        ...
                        return await blal.send_message(BOTLOG_CHATID, done_error, parse_mode="html")
        ...
        await blal.send_message(BOTLOG_CHATID, done, parse_mode="html")


@blal.dev_cmd(pattern="سحب?(?: |$)(.*)")
async def _(Matrixal):
    chat_id = normalize_chat_id(Matrixal.chat_id)
...
@blal.dev_cmd(pattern="ضف (الاعضاء|المشرفين|البوتات)?$")
async def _(Matrixal):
    if not Matrixal.is_group:
        return await edit_delete(Matrixal, "**ايا مطـي .. هـذه ليست مجموعـة ؟!**")
    chat_id = normalize_chat_id(Matrixal.chat_id)
...
@blal.dev_cmd(pattern="ايقاف الاضافه$")
async def _(Matrixal):
    if not Matrixal.is_group:
...
async def Matrixal_join(event): 
    ...
    if link.startswith('https://t.me/+'):
        try:
            ...
            result = await blal(ImportChatInviteRequest(link.strip()))
            return await zzz.edit("**- تم الانضمـام .. بنجـاح  ✓**")
        except Exception as e:
            return await zzz.edit("**- خطـأ:**\n" + str(e))
    elif link.startswith('@'):
        try:
            get_entity_must_join = await blal.get_entity(link)
            result = await blal(JoinChannelRequest(get_entity_must_join.id))
            return await zzz.edit("**- تم الانضمـام .. بنجـاح  ✓**")
        except Exception as e:
            return await zzz.edit("**- خطـأ:**\n" + str(e))
...
async def Matrixal_leave(event): 
    ...
    if link.startswith('https://t.me/+'):
        ...
        try:
            get_entity_must_join = await blal.get_entity(link)
            result = await blal(LeaveChannelRequest(get_entity_must_join.id))
            return await zzz.edit("**- تم المغـادرة .. بنجـاح  ✓**")
        except Exception as e:
            return await zzz.edit("**- خطـأ:**\n" + str(e))
    elif link.startswith('https://t.me/'):
