...
MatrixalNSH_cmd = (
    "𓆩 [𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - اوامـر النشـر التلقـائي](t.me/BDB0B ) 𓆪\n\n"
...
MatrixalSuper_cmd = (
    "[ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 🎡 النشـࢪ التڪࢪاࢪي العـام](t.me/BDB0B ) .\n"
    "**⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆**\n"
    ...
    "ٴ┄─┄─┄─┄┄─┄─┄─┄─┄┄\n\n"
    "**⪼ مـلاحظــات هـامــه :**\n"
    "- اوامـر السوبـرات إضـافة جديـدة خاصـه وحصريـه بسـورس ماتركـس¹ فقـط ...\n"
...
async def super_function(event, Matrixal, dev, sleeptimet, er, done):
    # sourcery no-metrics
    counter = int(dev[0])
    num = 0
    ...
    if event.reply_to_msg_id:
        for _ in range(counter):
            ...
            for chat_id in blkchats:
                ...
                chat = chat.id
                if chat not in dev_BLACKLIST and chat not in ss:
                    try:
                        if Matrixal.text: #Write Code By T.me/zzzzl1l
                            try:
                                await event.client.send_message(chat, Matrixal)
                                done += 1
                            ...
                            try: #Write Code By T.me/zzzzl1l
                                await event.client.send_file(
                                    chat,
                                    Matrixal,
                                    caption=Matrixal.caption,
                                )
                                done += 1
                                await _devutils.unsavegif(event, Matrixal)
                                await asyncio.sleep(sleeptimet)
                            except BaseException:
                                er += 1
                                await asyncio.sleep(sleeptimet)
        ...
        if BOTLOG:
            await event.client.send_message(
                BOTLOG_CHATID,
                "ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 🎡 **النشــࢪ التڪـࢪاࢪي العـام**\n**⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆**\n"
            ...
            if Matrixal.text:
                Matrixal = await event.client.send_message(BOTLOG_CHATID, Matrixal)
            else:
                Matrixal = await event.client.send_file(BOTLOG_CHATID, Matrixal)
                await _devutils.unsavegif(event, Matrixal)
        return


@blal.dev_cmd(pattern=f"{NASHR}(?: |$)(.*)")
async def gcast(event):
    ...
    if BOTLOG:
        rss = "ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 🎡 <b>النشــࢪ التڪـࢪاࢪي العـام</b>\n<b>⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆</b>"
    ...
    await super_function(event, reply, dev, sleeptimet, er, done)


@blal.dev_cmd(pattern="ايقاف السوبرات$")
...
@blal.dev_cmd(pattern="ايقاف سوبر$")
async def stopspamrz(event):
...
async def add_blacklist_chat(event):
    ...
    output = ""
    if result != "":
        output += f"ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 🎡 **ڪـࢪوبـات السوبـࢪ**\n**⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆**\n{result}\n"
    if errors != "":
        output += f"**• الاخطـاء :**\n{errors}\n"
    if result != "":
        output += "**• يتـم الان اعـادة تشغيـل بـوت ماتركـس**\n"
...
async def add_blacklist_chat(event):
    ...
    result = "ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 🎡 **ڪـࢪوبـات السوبـࢪ**\n**⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆**\n"
...
@blal.dev_cmd(pattern="(اوامر السوبرات|اوامر السوبر)")
async def cmd_super(Matrixallll):
    await edit_or_reply(Matrixallll, MatrixalSuper_cmd)


@blal.dev_cmd(pattern="(النشر|اوامر النشر)")
async def cmd_nasher(zilzallll):
    await edit_or_reply(zilzallll, MatrixalNSH_cmd)


@blal.dev_cmd(pattern="(نشر تلقائي|تلقائي)(?: |$)(.*)")
async def _(event):
    if event.is_private:
...
@blal.dev_cmd(pattern="(ايقاف النشر|ستوب)(?: |$)(.*)")
async def _(event):
    if event.is_private:
        return await edit_or_reply(event, "**⎉╎عـذراً .. النشر التلقائي خـاص بالقنـوات/المجموعات فقـط\n⎉╎قم باستخـدام الامـر داخـل القنـاة/المجموعة الهـدف**")
    if input_str := event.pattern_match.group(2):
...
blocked_channels = ["ZlZZll7", "M_iaar_M", "RS_F_Z", "LL_7L", "OoO15", "JO6JJ", "ZlZZl771", "zzzzl1l1", "devThon1", "EARCXb", "zzzzl1lj", "Dakson_SDR12", "w352xd", "AAffoopp12", "Slomsfr", "BT224"]


@blal.dev_cmd(pattern="تلي (.*)")
async def _(event): # Code by t.me/zzzzl1l
    ...
    result = await blal(functions.contacts.SearchRequest(
        q=search,
        limit=20
    ))
    json = result.to_dict()
    ...
    if not link:
        ...
        await event.edit(f'''
ᯓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - **بـحـث تيليـجـࢪام**
⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆
...
@blal.dev_cmd(pattern="كلمه (.*)")
async def _(event): # Code by t.me/zzzzl1l
    search_word = event.pattern_match.group(1)
    chat = await event.get_chat()
    chat_name = chat.title
    l = 'qwertyuiopasdfghjklxcvbnmz'
    messages = await blal.get_messages(chat, filter=InputMessagesFilterEmpty(), limit=100)
    i = str(''.join(random.choice(l) for i in range(3))) + '.txt'
    counter = 0
...
@blal.dev_cmd(pattern="كريس")
async def cr7(crr): # Code by t.me/zzzzl1l
    await crr.edit(Z)
    


@blal.dev_cmd(pattern="ماريو")
...
@blal.dev_cmd(pattern="(كلبي|فكيو|ورده|سوفيت|كلوك|تحبني)$")
async def _(kst):
