...
Gdev_IMG = gtg_fanan or gtg_fanan2 or gtg_fanan3 or gtg_fanan4 or gtg_fanan5
...
async def who(event):
    ...
    replied_user = await get_user_from_event(event)
    try:
        Gdev_IMG, caption = await ifetch_info(replied_user, event)
    except AttributeError:
        return await edit_or_reply(dev, "**- لـم استطـع العثــور ع الشخــص**")
    message_id_to_reply = event.message.reply_to_msg_id
    if not message_id_to_reply:
