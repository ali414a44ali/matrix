...
from Matrix import Convert, blal
...
def pack_name(userid, pack, is_anim, is_video):
    if is_anim:
        return f"Matrix_{userid}_{pack}_anim"
    if is_video:
        return f"Matrix_{userid}_{pack}_vid"
    return f"Matrix_{userid}_{pack}"
...
async def kang(args):  # sourcery no-metrics
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            if message.media.document.mime_type == "video/webm":
                ...
                devevent = await edit_or_reply(args, "__⌛ Downloading..__")
                sticker = await animator(message, args, devevent)
    ...
    if photo:
        ...
        if (
            ...
            await devevent.edit("`Brewing a new Pack...`")
            async with args.client.conversation("@Stickers") as conv:
...
async def pack_kang(event):  # sourcery no-metrics
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await edit_delete(devevent, "`Unsupported File!`")
            return
        if photo:
...
async def pussycat(args):
    ...
    await devevent.edit("`Hold on, making sticker...`")
    async with args.client.conversation("@Stickers") as conv:
        otherpack, packname, emoji = await newpacksticker(
            devevent,
            conv,
...
async def pic2packcmd(event):
    ...
    image = await _devtools.media_to_pic(devevent, reply, noedits=True)
    if image[1] is None:
        return await edit_delete(
    ...
    newimg.save(new_img)
    new_img.seek(0)
    devevent = await event.edit("__Making the pack.__")
    async with event.client.conversation(chat) as conv:
        ...
        for im in images:
            ...
            i += 1
            await devevent.edit(f"__Making the pack.\nProgress: {i}/{len(images)}__")
...
async def cb_sticker(event):
    ...
    devevent = await edit_or_reply(event, "`Searching sticker packs....`")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await edit_delete(devevent, "`No results found :(.`", 5)
