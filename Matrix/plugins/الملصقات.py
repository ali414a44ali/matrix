...
# ZThon UsetBot T.me/devThon
# Devolper Matrixal T.me/zzzzl1l
import asyncio
import contextlib
import io
import math
...
def char_is_emoji(character):
    return character in devemoji.UNICODE_EMOJI["en"]
...
async def delpack(devevent, conv, args, packname):
    try:
        await conv.send_message("/delpack")
    except YouBlockedUserError:
        await blal(unblock("stickers"))
        await conv.send_message("/delpack")
...
async def add_to_pack(
    ...
    while ("50" in x.message) or ("120" in x.message) or vtry:
        ...
        await devevent.edit(f"**⌔∮التبديـل الى الحزمـه** {pack} **بسبب امتـلاء الحزمـه الحاليـه ..** ")
        await conv.send_message(packname)
        x = await conv.get_response()
...
async def kang(args):  # sourcery no-metrics
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            devevent = await edit_or_reply(args, f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            ...
            if message.media.document.mime_type == "video/webm":
                ...
                for attribute in attributes:
                    if isinstance(attribute, DocumentAttributeSticker):
                        if message.media.document.size / 1024 > 255:
                            ...
                            sticker = await animator(message, args, devevent)
                            await edit_or_reply(
                                devevent, f"`{random.choice(KANGING_STR)}`"
                            )
                        else:
                            devevent = await edit_or_reply(
                ...
                devevent = await edit_or_reply(args, "__⌛ جـارِ ...__")
                sticker = await animator(message, args, devevent)
                await edit_or_reply(devevent, f"`{random.choice(KANGING_STR)}`")
        else:
    ...
    if photo:
        ...
        if len(splat) == 2:
            if char_is_emoji(splat[0][0]):
                if char_is_emoji(splat[1][0]):
                    return await devevent.edit("**- ارسـل الامـر**  `.معلومات الملصق`  **بالـرد ع الملصـق للتحـقق ...**")
        ...
        if (
            ...
            await devevent.edit("**⌔∮ جارِ التحظير لحزمة جديده...**")
            async with args.client.conversation("@Stickers") as conv:
...
async def pack_kang(event):
    ...
    for message in reqd_sticker_set.documents:
        if "image" in message.mime_type.split("/"):
            ...
            await edit_delete(devevent, "**⪼ ملـف غيـر مدعـوم ؟!**")
            return
        if photo:
            ...
            if (
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, devpackname = await newpacksticker(
                        devevent,
                        conv,
                        cmd,
                        event,
                ...
                async with event.client.conversation("@Stickers") as conv:
                    pack, devpackname = await add_to_pack(
                        devevent,
                        conv,
                        event,
                        packname,
            ...
            if devpackname is None:
                return
            if devpackname not in blablapacks:
                blablapacks.append(devpackname)
...
async def pussycat(args):
    ...
    if message and message.media:
        if "video/mp4" in message.media.document.mime_type:
            devevent = await edit_or_reply(args, "**- جـارِ التحويـل . . .**")
            sticker = await animator(message, args, devevent)
            await edit_or_reply(devevent, f"`{random.choice(KANGING_STR)}`")
        else:
    ...
    await devevent.edit("**⪼ جـارِ صنـع الملصـق . . .**")
    async with args.client.conversation("@Stickers") as conv:
        otherpack, packname, emoji = await newpacksticker(
            devevent,
            conv,
...
async def pic2packcmd(event):
    ...
    devevent = await edit_or_reply(event, "__🔪Cropping and adjusting the image...__")
    try:
    ...
    newimg.save(new_img)
    new_img.seek(0)
    devevent = await event.edit("**- جـارِ صنـع حزمـة ملصقـات . . .**")
    async with event.client.conversation(chat) as conv:
        ...
        for im in images:
            ...
            i += 1
            await devevent.edit(f"**- جـارِ صنـع حزمـة ملصقـات . . .**\n**- العمليـة :** {i}/{len(images)}")
...
async def get_pack_info(event):
    ...
    if not isinstance(stickerset_attr, DocumentAttributeSticker):
        return await devevent.edit("**هذا ليس ملصقًا. الرد على ملصق.**")
    get_stickerset = await event.client(
        GetStickerSetRequest(
            InputStickerSetID(
                id=stickerset_attr.stickerset.id,
...
@blal.dev_cmd(pattern="ملصقات ?([\\s\\S]*)")
async def cb_sticker(event):
    ...
    devevent = await edit_or_reply(event, "**- جـارِ البحث عـن حـزم ملصقـات . . .**")
    scraper = cloudscraper.create_scraper()
    text = scraper.get(combot_stickers_url + split).text
    soup = bs(text, "lxml")
    results = soup.find_all("div", {"class": "sticker-pack__header"})
    if not results:
        return await edit_delete(devevent, "**- لايوجـد نتائـج بحث عن {split} :(**\n**- جـرب البحـث عن اسـم آخـر**", 5)
    ...
    await devevent.edit(reply)


#Code by T.me/zzzzl1l
@blal.dev_cmd(pattern="حذف_ملصق$")
async def Matrixal_gif(event):
    ...
    dev = await edit_or_reply(event, "**⎉╎جـارِ حـذف الملصـق مـن حزمتـك ...**")
    async with borg.conversation(chat) as conv: #Code by T.me/zzzzl1l
        try:
            ...
            await dev.delete()
            await borg.send_message(event.chat_id, devthon)
        except YouBlockedUserError:
            await blal(unblock("Stickers"))
            await conv.send_message("/start")
            await conv.get_response()
