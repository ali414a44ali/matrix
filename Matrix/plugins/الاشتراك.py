...
zilzal = blal.uid
dev_dev = (79291869416, 7291869416, 7291869416)
LOGS = logging.getLogger(__name__)
Matrixaal = False
MUTE_RIGHTS = ChatBannedRights(until_date=None, send_messages=True)
UNMUTE_RIGHTS = ChatBannedRights(until_date=None, send_messages=False)
ANTI_DDDD_devTHON_MODE = ChatBannedRights(
...
@blal.dev_cmd(pattern="(ضع اشتراك الخاص|وضع اشتراك الخاص)(?: |$)(.*)")
async def _(event):
...
@blal.dev_cmd(pattern="(تفعيل اشتراك الخاص|تفعيل الاشتراك خاص)")
...
@blal.dev_cmd(pattern="(تعطيل اشتراك الخاص|تعطيل الاشتراك الخاص)")
...
async def fp(event):
    ...
    Matrixal = (await event.get_sender()).id
...
@blal.dev_cmd(pattern="(ضع اشتراك الكروب|وضع اشتراك الكروب) ?(.*)")
async def fs(event):
...
@blal.dev_cmd(incoming=True, func=lambda e: e.is_group, edited=False, forword=None)
async def fg(event):
    chat_db = is_fsub(event.chat_id)
    if not chat_db:
        return
    chat_id = event.chat_id
    dev_dev = (7291869416, 7291869416, 925972505)
        ...
        try:
            ...
            if rip is False:
                await bot.send_message(
                    event.chat_id, f"[ᯓ 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗭𝗧𝗛𝗢𝗡 - الاشتࢪاك الإجباࢪي](t.me/BDB0B)\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n\n⌔╎**مࢪحبـاً عـزيـزي 🫂** [{sender.first_name}](tg://user?id={sender.id}) \n⌔╎**لـ الغـاء كتمـك 🔊**\n⌔╎**يُࢪجـى الإشتـࢪاك بالقنـاة @{channel} **", link_preview=False
                )
                await event.delete()
        except:
...
@blal.dev_cmd(pattern="تعطيل اشتراك الكروب$")
