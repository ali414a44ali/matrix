...
# All Rights Reserved for "dev-Thon - ZelZal" "زلـزال الهيبـه"
ANTI_DDDD_BiLaL_MODE = ChatBannedRights(
...
async def _(event):
    ...
    if input_str == "الكل":
        ...
        update_lock(dev_id, "button", True)
        update_lock(dev_id, "inline", True)
        update_lock(dev_id, "video", True)
        update_lock(dev_id, "sticker", True)
        update_lock(dev_id, "voice", True)
...
async def _(event):
   ...
   # All Rights Reserved for "dev-Thon - ZelZal" "زلـزال الهيبـه"
    ...
    chat_per = (await event.get_chat()).default_banned_rights
    if input_str == "البوتات":
        update_lock(dev_id, "bots", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الدخول":
        update_lock(dev_id, "location", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الاضافه":
        update_lock(dev_id, "contact", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "التوجيه":
        update_lock(dev_id, "forward", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الفارسيه" or input_str == "دخول الايران":
        update_lock(dev_id, "egame", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الفشار":
        update_lock(dev_id, "rtl", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الروابط":
        update_lock(dev_id, "url", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الميديا":
        update_lock(dev_id, "game", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "تعديل الميديا":
        update_lock(dev_id, "document", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "المعرفات":
        update_lock(dev_id, "button", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الانلاين":
        update_lock(dev_id, "inline", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
    if input_str == "الكل":
        update_lock(dev_id, "bots", False)
        update_lock(dev_id, "game", False)
        ...
        update_lock(dev_id, "button", False)
        update_lock(dev_id, "inline", False)
        update_lock(dev_id, "video", False)
        update_lock(dev_id, "sticker", False)
        update_lock(dev_id, "voice", False)
        return await edit_or_reply(event, "**⎆ تـم فتـح** {} **بنجـاح ✅ 𓆰•**".format(input_str))
...
async def _(event):
   ...
   # All Rights Reserved for "dev-Thon - ZelZal" "زلـزال الهيبـه"
    res = ""
    current_dev_locks = get_locks(event.chat_id)
    if not current_dev_locks:
        res = "**⎆ حـالة الحمـايه لـ هـذه المجمـوعـة :**"
    else:
        res = "**- فيمـا يلـي إعـدادات حمـاية المجمـوعـة :** \n"
        ubots = "❌" if current_dev_locks.bots else "✅"
        uegame = "❌" if current_dev_locks.egame else "✅"
        urtl = "❌" if current_dev_locks.rtl else "✅"
        uforward = "❌" if current_dev_locks.forward else "✅"
        ubutton = "❌" if current_dev_locks.button else "✅"
        uurl = "❌" if current_dev_locks.url else "✅"
        ugame = "❌" if current_dev_locks.game else "✅"
        udocument = "❌" if current_dev_locks.document else "✅"
        ulocation = "❌" if current_dev_locks.location else "✅"
        ucontact = "❌" if current_dev_locks.contact else "✅"
        ubutton = "❌" if current_dev_locks.button else "✅"
        uinline = "❌" if current_dev_locks.inline else "✅"
        res += f"**⎆  البوتات :** {ubots}\n"
...
@blal.dev_cmd(incoming=True, forword=None)
    ...
    if is_locked(dev_id, "game") and event.message.media:
        if zelzal == malath or await is_admin(event, zelzal) or zelzal in dev_dev:
	        ...
	        try:
	            ...
	            await event.client(
	                EditBannedRequest(
	                    event.chat_id, event.sender_id, ANTI_DDDD_BiLaL_MODE
	                )
	            )
	        except Exception as e:
	            await event.reply(
	            ...
	            update_lock(dev_id, "game", False)
    if is_locked(dev_id, "forward") and event.fwd_from:
        if zelzal == malath or await is_admin(event, zelzal) or zelzal in dev_dev:
            return
        else:
	        try:
	            ...
	            update_lock(dev_id, "forward", False)
    if is_locked(dev_id, "button") and "@" in hhh:
        if zelzal == malath or await is_admin(event, zelzal) or zelzal in dev_dev:
            return
        else:
	        try:
    ...
    if is_locked(dev_id, "egame") and ("فارسى" in hhh or "خوببی" in hhh or "میخوام" in hhh or "کی" in hhh or "پی" in hhh or "گ" in hhh or "خسته" in hhh or "صكص" in hhh or "راحتی" in hhh or "بیام" in hhh or "بپوشم" in hhh or "گرمه" in hhh or "چ" in hhh or "چه" in hhh or "ڬ" in hhh or "ٺ" in hhh or "چ" in hhh or "ڿ" in hhh or "ڇ" in hhh or "ڀ" in hhh or "ڎ" in hhh or "ݫ" in hhh or "ژ" in hhh or "ڟ" in hhh or "۴" in hhh or "زدن" in hhh or "دخترا" in hhh or "كسى" in hhh or "مک" in hhh or "خالى" in hhh or "ݜ" in hhh or "ڸ" in hhh or "پ" in hhh or "بند" in hhh or "عزيزم" in hhh or "برادر" in hhh or "باشى" in hhh or "ميخوام" in hhh or "خوبى" in hhh or "ميدم" in hhh or "كى اومدى" in hhh or "خوابيدين" in hhh):
        if zelzal == malath or await is_admin(event, zelzal) or zelzal in dev_dev:
	        ...
	        try:
	            ...
	            update_lock(dev_id, "egame", False)
    if is_locked(dev_id, "url") and "http" in hhh:
        if zelzal == malath or await is_admin(event, zelzal) or zelzal in dev_dev:
            return
        else:
	        try:
	            ...
	            update_lock(dev_id, "url", False)
    if is_locked(dev_id, "inline") and event.message.via_bot:
        if zelzal == malath or await is_admin(event, zelzal) or zelzal in dev_dev:
            return
        else:
	        try:
...
# Copyright (C) 2022 dev-Thon
@blal.on(events.MessageEdited)
async def check_edit_media(event):
    ...
    if event.is_group: #Write Code By T.me/zzzzl1l
        ...
        if not admin and not creator:
            return
    dev_dev = (7291869416, 6583951825, 1895219306, 7291869416, 6583951825)  #Write Code By T.me/zzzzl1l
    zelzal = event.sender_id
    malath = blal.uid
    hhh = event.message.text
    dev_id = event.chat_id
    user = await event.get_sender()
    if is_locked(dev_id, "document") and event.message.media: #Write Code By T.me/zzzzl1l
        if zelzal == malath or zelzal in dev_dev:
            return
        else:
	        try:
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "dev-Thon - ZelZal" "زلـزال الهيبـه"
    dev_dev = (7291869416, 6583951825, 1895219306, 7291869416, 6583951825)
    malath = blal.uid
    if not is_locked(event.chat_id, "contact"):
        return
    if event.user_added:
        devy = await event.client.get_entity(event.user_id)
        zelzal_by = event.action_message.sender_id
        dev = await event.client.get_permissions(event.chat_id, zelzal_by)
        is_ban_able = False
        ...
        for user_id in added_users:
            ...
            if event.user_added:
                is_ban_able = True
                if zelzal_by == malath or dev.is_admin or zelzal_by in dev_dev:
                    return
                else:
	                try:
	                    await event.client(
...
# Copyright (C) 2022 dev-Thon - كــود قفــل دخــول الايــران
@blal.on(events.ChatAction())
async def _(event):
    if not event.is_private:
        chat = await event.get_chat()
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "dev-Thon - BDB0B" "زلـزال الهيبـه"
    dev_dev = (1895219306, 7291869416, 6583951825)
    ...
    if event.user_joined: 
        ...
        if event.user_joined and ("ژ" in first or "چ" in first or "۴" in first or "مهسا" in first or "sara" in first or "گ" in first or "نازنین" in first or "آسمان" in first or "ڄ" in first or "پ" in first or "Sanaz" in first or "𝓈𝒶𝓇𝒶" in first or "سارة" in first or "GIRL" in first or " Lady " in first or "فتاة" in first or "👅" in first or "سمانه" in first or "بهار" in first or "maryam" in first or "👙" in first or "هانیه" in first or "هستی" in first or "💋" in first or "ندا" in first or "Mina" in first or "خانم" in first or "ایناز" in first or "مبینا" in first or "امینی" in first or "سرنا" in first or "اندیشه" in first or "لنتكلم" in first or "دریا" in first or "زاده" in first or "نااز" in first or "ناز" in first or "بیتا" in first or "سكس" in first or "💄" in first or "اعرب" in first or "أعرب" in first or "قحب" in first or "قحاب" in first or "عراب" in first or "مكود" in first or "عربك" in first or "مخنث" in first or "مخنوث" in first or "فتال" in first or "زاني" in first or "زنا" in first or "لقيط" in first or "بنات شوارع" in first or "بنت شوارع" in first or "نيك" in first or "منيوك" in first or "منيوج" in first or "نايك" in first or "قواد" in first or "زبي" in first or "ايري" in first or "ممحو" in first or "بنت شارع" in first or " است " in first or "اسات" in first or "زوب" in first or "عيير" in first or "املس" in first or "مربرب" in first or " خول " in first or "عرص" in first or "قواد" in first or "اهلاتك" in first or "جلخ" in first or "شرمو" in first or "فرك" in first or "رهط" in first):
            is_ban_able = True
            if devy.id in dev_dev:
                return
            else:
	            try:
	                await event.client(
	                        functions.channels.EditBannedRequest(
	                            event.chat_id, devy.id, rights
	                        )
	                    )
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "dev-Thon - BDB0B" "زلـزال الهيبـه"
    dev_dev = (7291869416, 6583951825, 1895219306, 126045030)
...
async def _(event):
    ...
    if event.user_added:
        ...
        for user_id in added_users:
            ...
            if user_obj.bot:
                is_ban_able = True
                if zelzal_by == malath or zelzal_by in dev_dev:
                    return
                else:
	                try:
	                    await event.client(
...
# Copyright (C) 2022 dev-Thon
@blal.dev_cmd(pattern=f"البوتات ?(.*)")
async def zelzal(dev):
    con = dev.pattern_match.group(1).lower()
    del_u = 0
    del_status = "**⎆ مجمـوعتك/قناتـك في أمـان ✅.. لاتوجـد بوتـات في هذه المجمـوعـة ༗**"
    if con != "طرد":
        event = await edit_or_reply(dev, "**⎆ جـاري البحـث عن بوتات في هـذه المجمـوعـة ...🝰**")
        async for user in dev.client.iter_participants(dev.chat_id):
            if user.bot:
                del_u += 1
                await sleep(0.5)
        if del_u > 0:
        ...
        return
    # All Rights Reserved for "dev-Thon - ZelZal" "زلـزال الهيبـه"
    chat = await dev.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
    if not admin and not creator:
        await edit_delete(dev, "**⎆ عـذࢪًا .. احتـاج الى صلاحيـات المشـرف هنـا**", 5)
        return
    event = await edit_or_reply(dev, "**⎆ جـارِ طـرد البوتـات من هنـا ...⅏**")
    del_u = 0
    del_a = 0
    async for user in dev.client.iter_participants(dev.chat_id):
        if user.bot:
            try:
                await dev.client.kick_participant(dev.chat_id, user.id)
                await sleep(0.5)
                del_u += 1
    ...
    if BOTLOG:
        await dev.client.send_message(
            BOTLOG_CHATID,
            f"#طـرد_البوتـات\
            \n ⎆ {del_status}\
            \n ⎆ الـدردشـة: {dev.chat.title}(`{dev.chat_id}`)",
        )
