...
async def on_new_private_message(event):
    ...
    zel_dev = (72918694163, 72918694169)
    ...
    if event.chat_id == 7291869416 or event.chat_id == 7291869416 or event.chat_id == 7291869416 or event.chat_id == 7291869416:
...
@blal.dev_cmd(outgoing=True, func=lambda e: e.is_private, edited=False, forword=None)
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"show_pmpermit_options")))
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"to_enquire_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"to_request_something")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"to_chat_with_my_master")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
@blal.tgbot.on(CallbackQuery(data=re.compile(rb"to_spam_my_master_inbox")))
async def on_plug_in_callback_query_handler(event):
    if event.query.user_id == event.client.uid:
        text = "⤶ عـذراً سيـدي ، هـذه الـخـيـارات لـلـمـسـتـخـدم الـذي يـراسـلـك 🤷🏻‍♂"
        return await event.answer(text, cache_time=0, alert=True)
...
@blal.dev_cmd(pattern="بلوك(?:\s|$)([\s\S]*)")
...
@blal.dev_cmd(pattern="الغاء بلوك(?:\s|$)([\s\S]*)")
...
@blal.dev_cmd(pattern="المقبولين$")
