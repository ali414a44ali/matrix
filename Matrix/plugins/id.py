...
async def ch_info(event): # كتابة الكود بواسطة زلزال - @ZlZZ7
...
async def zzz_info(zthon_user, event):
    ...
    if mypremium == True:
        ...
        ZThon += f'<a href="emoji/5832334750655715478">❤️</a>:\n'
        ZThon += f"<b>- الاسـم    ⤎  </b> "
        ZThon += f'<a href="tg://user?id={user_id}">{full_name}</a> '
        if zilzal == True:
            ZThon += f'<a href="emoji/{emoji_id}">❤️</a>'
        ZThon += f"\n<b>- الايــدي  ⤎ </b> <code>{user_id}</code>"
        ZThon += f"\n<b>- اليـوزر    ⤎  {username}</b>\n"
        if zilzal == True:
            ZThon += f"<b>- الحساب ⤎  بـريميـوم</b>"
            ZThon += f'<a href="emoji/5832422209074762334">❤️</a>\n'
        ZThon += f"<b>- الإنشـاء  ⤎</b>  {zzzsinc}  " 
        ZThon += f'<a href="emoji/5472026645659401564">❤️</a>'
        return ZThon
    else:
        ################# Dev ZilZal #################
...
@blal.dev_cmd(pattern="الانشاء(?: |$)(.*)")
async def zelzalll(event):
    dev = await edit_or_reply(event, "**- جـارِ جلب المعلومـات . . .**")
    ...
    if mypremium == True:
        try:
            ...
            await dev.edit("**- غيـر معلـوم او هنـاك خطـأ ؟!**", parse_mode="html")
    else:
        try:
            await event.client.send_message(
                event.chat_id,
...
@blal.dev_cmd(pattern="الصور(?:\\s|$)([\\s\\S]*)")
async def potocmd(event):
    uid = "".join(event.raw_text.split(maxsplit=1)[1:])
...
@blal.dev_cmd(pattern="اضف كليشة (الايدي|ايدي|الفحص|فحص|الحماية|الحمايه|الخاص) ?(.*)")
async def variable(event):
    input_str = event.pattern_match.group(1)
    reply = await event.get_reply_message()
    vinfo = reply.text
    dev = await edit_or_reply(event, "**⎉ جـاري اضـافة الكليشـة الـى بـوتك ...**")
    if input_str == "الايدي" or input_str == "ايدي":
        variable = "ZID_TEMPLATE"
        await asyncio.sleep(1.5)
        if gvarstatus("ZID_TEMPLATE") is None:
            await dev.edit("**⎉ تم تغييـر كليشـة {} بنجـاح ☑️**\n**⎉ الكليشـة الجـديده** \n {} \n\n**⎉ الان قـم بـ ارسـال الامـر ↶** `.ايدي` **لـ التحقـق مـن الكليشـة . .**".format(input_str, vinfo))
        else:
            await dev.edit("**⎉ تم اضـافـة كليشـة {} بنجـاح ☑️**\n**⎉ الكليشـة المضـافه** \n {} \n\n**⎉ الان قـم بـ ارسـال الامـر ↶** `.ايدي` **لـ التحقـق مـن الكليشـة . .**".format(input_str, vinfo))
        ...
        if input_str:
            return await dev.edit("**⎉ عـذࢪاً .. لايوجـد هنالك فـار بإسـم {} ؟!.. ارسـل (.اوامر الفارات) لـعرض قائمـة الفـارات**".format(input_str))
...
@blal.dev_cmd(pattern="اضف كليشه (الايدي|ايدي|الفحص|فحص|الحماية|الحمايه|الخاص) ?(.*)")
async def variable(event):
    input_str = event.pattern_match.group(1)
    reply = await event.get_reply_message()
    vinfo = reply.text
