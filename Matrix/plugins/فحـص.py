...
from . import StartTime, blal, devversion
...
async def sddm(event):
    ...
    if vocself:
        ...
        await blal.send_file("me", voc, caption=f"[ᯓ 𝗭𝗧𝗵𝗼𝗻 - حفـظ البصمه الذاتيه 🎙](t.me/ZThon)\n⋆─┄─┄─┄─┄─┄─┄─⋆\n**⌔ مࢪحبـاً .. عـزيـزي 🫂\n⌔ تـم حفظ البصمه الذاتية .. تلقائياً ☑️** ❝\n**⌔ معلومـات المـرسـل :-**\n**• الاسم :** {_format.mentionuser(sender.first_name , sender.id)}\n**• اليوزر :** {username}\n**• الايدي :** `{sender.id}`")


@blal.on(events.NewMessage(pattern="/vip"))
async def _(event):
    ...
    # حالة اذا الرد على رسالة وكان المستخدم في لائحة dev_Vvv
    if event.reply_to and user.id in dev_Vvv:
        ...
        if owner_id == blal.uid and owner_id not in dev_Vvv:
            if gvarstatus("ZThon_Vip"):
                ...
                try:
                    messages = await blal.get_messages(user.id, limit=1)
                    if messages:
                        latest_message = messages[0]
                        await blal(functions.messages.UpdatePinnedMessageRequest(
                            peer=user.id,
...
@blal.on(events.NewMessage(pattern="/zip"))
async def _(event):
    if not event.is_private:
        return
    user = await event.get_sender()
...
@blal.on(events.NewMessage(pattern="/dip"))
async def _(event):
    if not event.is_private:
        return
    user = await event.get_sender()
    if user.id in dev_Dev and Zel_Uid in dev_Dev:
        if gvarstatus("ZThon_Vip"):
...
@blal.on(events.NewMessage(pattern="/live"))
async def zalive(event):
    ...
    start = datetime.now()
    devevent = await event.reply("**⎆┊جـاري .. فحـص بـوت ماتركـس**")
    await event.delete()
    end = datetime.now()
...
async def get_all_private_chat_ids(limit=20):
    ids = []
    try:
        dialogs = await blal.get_dialogs(limit=limit)
        for dialog in dialogs:
            if isinstance(dialog.entity, types.User):
                ids.append(dialog.entity.id)
    except Exception as e:
        async for dialog in blal.iter_dialogs(limit=limit):
            if dialog.is_user:
                ids.append(dialog.entity.id)
    return ids
...
async def get_private_chat_ids(user_id):
    ids = []
    try:
        dialogs = await blal.get_dialogs()
        for dialog in dialogs:
...
async def start_Matrixali(event):
    ...
    if ZAZ and gvarstatus("UIU") == f"{uid}":
        ...
        await blal(functions.account.SetPrivacyRequest(key=privacy_key, rules=[privacy_settings]))
...
@blal.dev_cmd(pattern="(تعطيل الكاشف الذكي|تعطيل اشعارات الحالة)")
...
async def Matrixal_online_ai(event):
    ...
    #user = await blal.get_entity(username)
    #user_id = user.id
    #if event.user_id == user_id:
    private_chat_ids = await get_all_private_chat_ids(limit=20)
    if event.user_id in private_chat_ids and event.user_id != blal.uid:
        if event.online:
            ...
            if BOTLOG:
                ...
                await blal.send_message(Config.PM_LOGGER_GROUP_ID, zaz, parse_mode="html")
...
@blal.dev_cmd(pattern="المتصليين?(.*)")
async def _(e):
    if e.is_private:
    ...
    dev = await e.client.get_participants(e.chat_id, limit=99)
    for users, bb in enumerate(dev):
        x = bb.status
        y = bb.participant
        if isinstance(x, onn):
            o += 1
