...
MatrixalSelf_cmd = (
    "𓆩 [ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - حفـظ الذاتيـه 🧧](t.me/BDB0B) 𓆪\n\n"
    "**⪼** `.تفعيل الذاتيه`\n"
...
async def oho(event):
    ...
    await blal.send_file("me", pic, caption=f"**⎉╎تم حفـظ الصـورة الذاتيـه .. بنجـاح ☑️𓆰**")
    await event.delete()

@blal.dev_cmd(pattern="(تفعيل الذاتيه|تفعيل الذاتية)")
...
@blal.dev_cmd(pattern="(تعطيل الذاتيه|تعطيل الذاتية)")
...
@blal.on(events.NewMessage(func=lambda e: e.is_private and (e.photo or e.video) and e.media_unread))
async def sddm(event):
    ...
    if devself:
        ...
        await blal.send_file("me", pic, caption=f"[ᯓ 𝗦𝗼𝘂𝗿𝗰𝗲 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ - حفـظ الذاتيـه 🧧](t.me/BDB0B) .\n\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⌔╎مࢪحبـاً عـزيـزي المـالك 🫂\n⌔╎ تـم حفـظ الذاتيـة تلقائيـاً .. بنجـاح ☑️** ❝\n**⌔╎المـرسـل** {_format.mentionuser(sender.first_name , sender.id)} .")

#Code For T.me/zzzzl1l
@blal.dev_cmd(pattern="اعلان (\\d*) ([\\s\\S]*)")
async def selfdestruct(destroy):
    dev = ("".join(destroy.text.split(maxsplit=1)[1:])).split(" ", 1)
    message = dev[1]
    ttl = int(dev[0])
...
#Code For T.me/zzzzl1l
@blal.dev_cmd(pattern="إعلان (\\d*) ([\\s\\S]*)")
async def selfdestruct(destroy):
    dev = ("".join(destroy.text.split(maxsplit=1)[1:])).split(" ", 1)
    message = dev[1]
    ttl = int(dev[0])
...
async def gpost(event):
    ...
    for chat in channels_set:
        if event.media:
            ...
            await blal.send_message(int(chat), event.message)
