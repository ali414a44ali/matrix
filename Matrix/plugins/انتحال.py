...
from . import ALIVE_NAME, BOTLOG, BOTLOG_CHATID, blal, edit_delete, get_user_from_event
...
async def _(event):
    ...
    dev = await edit_or_reply(event, "**╮ جـارِ الكشـف عـن اليـوزر فـي المـزاد ...𓅫╰**")
    async with borg.conversation(chat) as conv: #Code by T.me/zzzzl1l
        try:
            ...
            await blal(unblock("GetUsernameBot"))
            await conv.send_message("/start")
            await conv.get_response()
...
async def Matrixal_gif(event):
    ...
    dev = await edit_or_reply(event, "**⎉╎جـارِ جلب معلومـات TikTok .. انتظر قليلا ▬▭**")
    ...
    if data:
        ...
        try:
            ...
            await edit_or_reply(dev, f"𓆩 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗧𝗶𝗸𝗧𝗼𝗸 𝗜𝗻𝗳𝗼 - **معلومـات تيـك تـوك** 𓆪\n⋆─┄─┄─┄─┄─┄─┄─┄┄─┄─⋆\n**• الاسـم :** {name}\n**• اليـوزر :** {username}\n**• الايـدي :** {id}\n**• المتابعـين :** {followers}\n**• يتابـع :** {following}\n**• الدولـة :** {acc} {country_emoji}\n**• تاريـخ إنشـاء الحسـاب :** {time}")
            await dev.delete()
    else:
        await dev.edit("**- لم استطـع الكشـف عـن الحسـاب او ان اليـوزر غيـر موجـود**")


@blal.dev_cmd(pattern="nn(?: |$)(.*)")
async def Matrixal_gif(event):
    Matrixal = event.pattern_match.group(1)
    reply = await event.get_reply_message()
    if not Matrixal and reply:
        Matrixal = reply.text
    if not Matrixal:
        return await edit_delete(event, "**- ارسـل (.nn) + يـوزر انستـا او بالـرد ع يـوزر انستـا**", 10)
    if Matrixal.startswith("@"):
        Matrixal = Matrixal.replace("@", "")
    dev = await edit_or_reply(event, "**⎉╎جـارِ جلب معلومـات الانستـا .. انتظر قليلا ▬▭**")
    ...
    async with borg.conversation(chat) as conv: # Code by T.me/zzzzl1l
        try:
            ...
            devthon = await conv.get_response()
            malath = devthon.text
            if "Username : " in devthon.text: # Code by T.me/zzzzl1l
                zzz = malath.replace("Username : `username`", f"**• اليـوزر :** `{Matrixal}`").replace("Name : ", "**• الاسـم :** ").replace("ID : ", "**• الايـدي :** ").replace("Bio : ", "**• البايـو :** ").replace("Posts : ", "**• المنشـورات :** ").replace("Followers : ", "**• المتابعيـن :** ").replace("Following : ", "**• المتابعهـم :** ").replace("\n\n", "\n")
                zz = f"𓆩 𝙈𝙖𝙏𝙍𝙞𝙭 ⌁ 𝗜𝗻𝘀𝘁𝗮 𝗜𝗻𝗳𝗼 - **معلومـات انستـا** 𓆪\n⋆─┄─┄─┄─┄─┄─┄─┄┄─┄─⋆\n{zzz}"
                try:
                    await borg.send_file(
                        event.chat_id,
                        devthon,
            ...
            await blal(unblock("instagram_information_users_bot"))
            await conv.send_message("/start")
            await conv.get_response()
            await conv.send_message(Matrixal)
            await asyncio.sleep(3)
