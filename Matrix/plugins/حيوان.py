...
async def fetch_info(replied_user, event):
    ...
    if x == 1:
       ...
       caption += f"<b> 𓆩 𝙎𝙊𝙐𝙍𝘾𝞝 𝗧𝗘𝗣𝗧𝗛𝗢𝗡 𓆪 </b> - @Matrix "
...
@blal.dev_cmd(pattern="حيوان(?: |$)(.*)")
async def who(event):
    dev = await edit_or_reply(event, "⇆")
    zel_dev = (7291869416, 5426390871)
    ...
    if replied_user.id in zel_dev:
       return await edit_or_reply(dev, "**- دي . . انـهُ احـد المطـورين . . انتـه الحيـوان ولك**")
    if replied_user.id == 7291869416 or replied_user.id == 1895219306 or replied_user.id == 2095357462:
       return await edit_or_reply(dev, "**- دي . . انـهُ المطـور . . انتـه الحيـوان ولك**")
