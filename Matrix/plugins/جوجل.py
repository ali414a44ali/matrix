# reverse search and google search  plugin for BiLaL
import contextlib
import os
import re
import urllib
...
from Matrix import BOTLOG, BOTLOG_CHATID, Convert, blal
...
async def gsearch(q_event):
    ...
    devevent = await edit_or_reply(q_event, "**- جـارِ البحـث في جوجــل...**")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return await edit_delete(devevent, f"**- خطـأ :**\n`{e}`", time=10)
    msg = ""
    for i in range(lim):
...
async def grs(event):
    ...
    if event.reply_to_msg_id:
        devevent = await edit_or_reply(event, "Pre Processing Media")
        ...
        if previous_message.media:
            ...
            the_location = google_rs_response.headers.get("Location")
        await devevent.edit("Found Google Result. Pouring some soup on it!")
        headers = {
        ...
        try:
            ...
            return await edit_delete(
                devevent, "`Sorry. I am unable to find similar images`"
            )
        end = datetime.now()
...
<i>fetched in {ms} seconds</i>""".format(
        ...
        devevent = event
    await edit_or_reply(devevent, OUTPUT_STR, parse_mode="HTML", link_preview=False)
