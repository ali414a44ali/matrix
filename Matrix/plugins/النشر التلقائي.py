...
ZelzalNSH_cmd = (
    "𓆩 [ اوامـر النشـر التلقـائي](t.me/Matrix) 𓆪\n\n"
...
@blal.dev_cmd(pattern="(نشر تلقائي|تلقائي)(?:\s|$)([\s\S]*)")
async def _(event):
...
# Copyright (C) 2022 dev-Thon . All Rights Reserved
@blal.dev_cmd(pattern="(ايقاف النشر|ستوب)(?:\s|$)([\s\S]*)")
async def _(event):
    if (event.is_private or event.is_group):
        return await edit_or_reply(event, "**⎉╎عـذراً .. النشر التلقائي خـاص بالقنـوات فقـط\n⎉╎قم باستخـدام الامـر داخـل القنـاة الهـدف**")
    if input_str := event.pattern_match.group(2):
