...
@blal.on(admin_cmd(pattern="(تجميع المليار x|تجميع مليار x)"))
async def _(event):
    if Matrix[0] == "yes":
        await event.edit("**𓆰 حـسنـًا .. تأكـد من انك مشتـرك بـ قنـوات الاشتـراك الاجبـاري لتجنب الأخطـاء @EEOBot**")
        channel_entity = await blal.get_entity(bot_username)
        ...
        for i in range(100):
            ...
            list = await blal(GetHistoryRequest(peer=channel_entity, limit=1,
                                                   offset_date=None, offset_id=0, max_id=0, min_id=0, add_offset=0, hash=0))
            msgs = list.messages[0]
            if msgs.message.find('**𓆰 لا يوجد قنوات في الوقت الحالي .. قم يتجميع النقاط بطريقه مـخـتـلفة**') != -1:
                await blal.send_message(event.chat_id, f"**𓆰 لا يـوجـد قنـوات بالبـوت حـاليًا ...**")
                break
            ...
            try:
                ...
                chs += 1
                await blal.send_message("**𓆰 تم بنجـاح الاشتـراك في {chs} قنـاة .❗**")
            except:
                await blal.send_message(event.chat_id, f"**𓆰 القنـاة رقـم {chs} خطـأ .. يمكـن تبنـدت**")
                break
        await blal.send_message(event.chat_id, "**𓆰 تم الانتهـاء مـن تجميـع النقـاط .. حاول من جديد في وقت آخر ✓**")
