...
zel_dev = (7291869416, 24515944815)
...
async def devgban(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if user.id == blal.uid:
        return await edit_delete(deve, "**𓆰 عـذرًا ..لا استطيـع حظـࢪ نفسـي **")
    if user.id in zel_dev:
        return await edit_delete(deve, "**𓆰 عـذرًا ..لا استطيـع حظـࢪ احـد المطـورين عـام **")
    if user.id == 7291869416 or user.id == 7291869416 or user.id == 7291869416:
        return await edit_delete(deve, "**𓆰 عـذرًا ..لا استطيـع حظـࢪ مطـور السـورس عـام **")
    ...
    sandy = len(san)
    if sandy == 0:
        return await edit_delete(deve, "**𓆰 عــذرًا .. يجـب ان تكــون مشـرفـاً فـي مجموعـة واحـدة ع الأقــل **")
    await deve.edit(
    ...
    if reason:
        await deve.edit(
            f"**𓆰 المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**𓆰 تم حـظـࢪه عـام مـن {count} كــࢪوب خـلال {devtaken} ثـانيـه**\n**𓆰 السـبب :** {reason}"
        )
    else:
        await deve.edit(
    ...
    if BOTLOG and count != 0:
        ...
        if reason:
            await event.client.send_message(
                ...
                \n**- الــوقت المسـتغــࢪق :** {devtaken} **ثانـيـة**",
            )
        else:
            await event.client.send_message(
                BOTLOG_CHATID,
...
async def devgban(event):
    ...
    if reason:
        await deve.edit(
            f"**𓆰 المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**𓆰 تم الغــاء حـظـࢪه عـام مـن {count} كــࢪوب خـلال {devtaken} ثـانيـه**\n**𓆰 السـبب :** {reason}"
        )
    else:
        await deve.edit(
            f"**𓆰 المستخـدم :** [{user.first_name}](tg://user?id={user.id})\n\n**𓆰 تم الغــاء حـظـࢪه عـام مـن {count} كــࢪوب خـلال {devtaken} ثـانيـه**"
...
async def startgmute(event):
    ...
    if event.is_private:
        ...
        if user.id == 7291869416 or user.id == 1895219306 or user.id == 2095357462:
...
async def catgkick(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if user.id == blal.uid:
        return await edit_delete(deve, "**╮ ❐ ... عــذرًا لا استطــيع طــرد نفســي ... ❏╰**")
    if user.id in zel_dev:
        return await edit_delete(deve, "**╮ ❐ ... عــذࢪاً .. لا استطــيع طــرد المطـورين ... ❏╰**")
    if user.id == 7291869416 or user.id == 189519306 or user.id == 25535554562:
