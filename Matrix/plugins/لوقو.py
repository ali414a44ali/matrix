...
Write file by Matrixal t.me/zzzzl1l
...
async def lg1(userevent):
    ...
    if gvarstatus("dev_FONTS") is None: # code by t.me/zzzzl1l
        dev_font = await get_font_file(userevent.client, "@T_Taiz")
    else:
        dev_font = gvarstatus("dev_FONTS")
    ...
    font = ImageFont.truetype(dev_font, font_size_)
...
async def lg1(userevent):
    ...
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    if gvarstatus("dev_FONTS") is None: # code by t.me/zzzzl1l
        dev_font = await get_font_file(userevent.client, "@S_SSQ")
    else: # code by t.me/zzzzl1l
        dev_font = gvarstatus("dev_FONTS")
    if userevent.reply_to_msg_id:
        rply = await userevent.get_reply_message()
